  ...
  try {
    ...
    if (!bucketExists) {
      return NextResponse.json(
        createApiResponse(null, 'none', `Bucket ${bucket} does not exist`),
        { status: 404 }
      );
    }
    ...
    if (episodeId) {
      ...
      if (!episode) {
        return NextResponse.json(
          createApiResponse(null, 'none', `Episode ${episodeId} not found`),
          { status: 404 }
        );
      }
    ...
    return NextResponse.json(
      createApiResponse(
        null,
        'none',
        `Failed to retrieve L1 episodes: ${error instanceof Error ? error.message : 'Unknown error'}`
      ),
      { status: 500 }
    );
  }
