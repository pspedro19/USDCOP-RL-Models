  ...
  try {
    ...
    if (!startDate || !endDate) {
      return NextResponse.json(
        createApiResponse(null, 'none', 'startDate and endDate are required', {
          message: 'Example: /api/pipeline/l0?startDate=2024-01-01&endDate=2024-12-31'
        }),
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        return NextResponse.json(
          createApiResponse(
            { count: data.length, data, source: 'minio' },
            'minio',
            undefined,
    ...
    try {
      ...
      if (timeSeries && timeSeries.length > 0) {
        ...
        return NextResponse.json(
          createApiResponse(
            { count: filteredData.length, data: filteredData, source: 'twelvedata' },
            'live',
            undefined,
      ...
      return NextResponse.json(
        createApiResponse(
          null,
          'none',
          `Failed to load data from both MinIO and TwelveData: ${apiError instanceof Error ? apiError.message : 'Unknown error'}`,
          { message: 'Try /api/pipeline/l0/raw-data for PostgreSQL data (92K+ records available)' }
        ),
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      createApiResponse(
        null,
        'none',
        `Failed to load L0 data: ${error instanceof Error ? error.message : 'Unknown error'}`
      ),
      { status: 500 }
    );
  }
