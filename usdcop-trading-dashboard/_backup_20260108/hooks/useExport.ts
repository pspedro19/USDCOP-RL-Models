...
import type { BacktestResult, ExcelSheetData, Position, Prediction, PipelineStatus } from '@/types';
...
export const useBacktestExport = () => {
  ...
  const exportToPDF = useCallback(async (backtestData: BacktestResult) => {
  ...
  const exportToCSV = useCallback((backtestData: BacktestResult) => {
    try {
      const exporter = new ProfessionalExportManager(exportConfigs.backtestReport);
  ...
  const exportToExcel = useCallback((backtestData: Partial<BacktestResult> & { monthlyReturns?: Record<string, unknown>[]; drawdowns?: Record<string, unknown>[] }) => {
    try {
      const exporter = new ProfessionalExportManager(exportConfigs.backtestReport);

      const sheets: ExcelSheetData[] = [
        { name: 'Summary', data: backtestData.id ? [backtestData as Record<string, unknown>] : [] },
        { name: 'Monthly Returns', data: backtestData.monthlyReturns || [] },
        { name: 'Trades', data: (backtestData.trades || []) as Record<string, unknown>[] },
...
export const useRiskExport = () => {
  ...
  const exportToPDF = useCallback(async (riskData: { positions?: Position[] }) => {
  ...
  const exportToCSV = useCallback((riskData: { positions?: Position[] }) => {
    try {
      const exporter = new ProfessionalExportManager(exportConfigs.riskReport);
...
export const useModelExport = () => {
  ...
  const exportToPDF = useCallback(async (modelData: { predictions?: Prediction[] }) => {
  ...
  const exportToCSV = useCallback((modelData: { predictions?: Prediction[] }) => {
    try {
      const exporter = new ProfessionalExportManager(exportConfigs.modelReport);
...
interface CorrelationData {
  ...
  multicollinearityIssues?: number;
  avgVif?: number;
  highCorrelationPairs?: CorrelationPair[];
  vifAnalysis?: VIFItem[];
  correlationMatrix?: Record<string, unknown>[];
}
...
export const useCorrelationExport = () => {
  ...
  const exportToPDF = useCallback(async (correlationData: CorrelationData) => {
      ...
      const tables: TableData[] = [
        {
          ...
          rows: correlationData.highCorrelationPairs?.map((pair) => [
            pair.feature1,
            pair.feature2,
        ...
        },
        {
          ...
          rows: correlationData.vifAnalysis?.map((item) => [
            item.feature,
            item.vif.toFixed(2),
  ...
  const exportToCSV = useCallback((correlationData: CorrelationData) => {
    try {
      const exporter = new ProfessionalExportManager({
...
interface PipelineData {
  ...
  layers?: PipelineStatus[];
}

export const usePipelineExport = () => {
  const exportToPDF = useCallback(async (pipelineData: PipelineData) => {
      ...
      const tables: TableData[] = [
        {
          ...
          rows: pipelineData.layers?.map((layer) => [
            layer.layer,
            layer.status,
  ...
  const exportToCSV = useCallback((pipelineData: PipelineData) => {
    try {
      const exporter = new ProfessionalExportManager({
