...
export class PipelineAPI {
  // ========== L0: Raw Data ==========
  static async getL0RawData(params?: QueryParams) {
    const query = new URLSearchParams(params as Record<string, string>).toString();
    const url = `${API_BASE_URLS.pipeline}/api/pipeline/l0/raw-data${query ? `?${query}` : ''}`;
    return fetchJSON<ApiResponse<L0RawData>>(url);
  }
  ...
  static async getL3Features() {
    ...
    return fetchJSON<ApiResponse<L3Features>>(url);
  }
  ...
  static async getL4QualityCheck() {
    ...
    return fetchJSON<ApiResponse<L4QualityCheck>>(url);
  }
  ...
  static async getL5Models() {
    ...
    return fetchJSON<ApiResponse<L5Models>>(url);
  }
  ...
  static async getL6BacktestResults(split: string = 'test') {
    ...
    return fetchJSON<ApiResponse<L6BacktestResults>>(url);
  }
}
...
export class TradingAnalyticsAPI {
  ...
  static async getMarketConditions(symbol: string = 'USDCOP', days: number = 30) {
    ...
    return fetchJSON<ApiResponse<Record<string, unknown>>>(url);
  }
...
export class TradingAPI {
  ...
  static async getPositions() {
    ...
    return fetchJSON<ApiResponse<Position[]>>(url);
  }
