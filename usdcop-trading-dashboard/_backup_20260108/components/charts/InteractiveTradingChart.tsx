...
import { CandlestickData as OHLCVCandle } from '@/types/trading';

/**
 * Line data for lightweight-charts indicators
 */
interface LightweightLineData {
  time: Time;
  value: number;
}
...
export const InteractiveTradingChart: React.FC<InteractiveTradingChartProps> = ({
  ...
  const calculateBollingerBands = (data: OHLCVCandle[]) => {
    const period = 20;
    const stdDev = 2;
    const upper: LightweightLineData[] = [];
    const middle: LightweightLineData[] = [];
    const lower: LightweightLineData[] = [];
  ...
  // Calculate EMA
  const calculateEMA = (data: OHLCVCandle[], period: number) => {
    const ema: LightweightLineData[] = [];
  ...
  const aggregateOHLC = (data: OHLCVCandle[], periodMinutes: number): OHLCVCandle[] => {
  ...
  const getTimeframeData = (allData: OHLCVCandle[], tf: typeof timeframe): OHLCVCandle[] => {
