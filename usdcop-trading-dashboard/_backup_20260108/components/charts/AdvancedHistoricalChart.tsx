    ...
    <div className={`bg-slate-900/50 backdrop-blur border border-slate-600/30 rounded-xl overflow-hidden ${isFullscreen ? 'fixed inset-0 z-50 bg-slate-950' : ''}`} style={{ height }}>
      ...
      <div className="px-6 py-2 border-t border-slate-700/50">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {(['1D', '1W', '1M', '3M', '1Y', 'ALL'] as const).map((preset) => (
              <motion.button
                key={preset}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
      ...
      <div className="px-6 pb-4">
        ...
        <div className="relative h-16 bg-slate-800/50 rounded-lg overflow-hidden">
          ...
          <motion.div
            ...
            dragElastic={0}
            onDrag={(e, info) => handleDrag(e, info, 'left')}
            onDragStart={() => setIsDraggingHandle('left')}
            onDragEnd={() => setIsDraggingHandle(null)}
            className="absolute top-0 bottom-0 w-2 bg-cyan-500 cursor-ew-resize z-10"
            style={{ left: `${leftHandle * 100}%` }}
          />
          ...
          <motion.div
            ...
            dragConstraints={containerRef}
            dragElastic={0}
            onDrag={(e, info) => handleDrag(e, info, 'right')}
            onDragStart={() => setIsDraggingHandle('right')}
            onDragEnd={() => setIsDraggingHandle(null)}
            className="absolute top-0 bottom-0 w-2 bg-cyan-500 cursor-ew-resize z-10"
            style={{ left: `${rightHandle * 100}%` }}
          />
          ...
          <motion.div
            ...
            dragConstraints={containerRef}
            dragElastic={0}
            onDrag={(e, info) => handleDrag(e, info, 'range')}
            onDragStart={() => setIsDraggingHandle('range')}
            onDragEnd={() => setIsDraggingHandle(null)}
            className="absolute top-0 bottom-0 border-t-2 border-b-2 border-cyan-500/50 cursor-move z-5"
            style={{
              left: `${leftHandle * 100}%`,
              width: `${(rightHandle - leftHandle) * 100}%`
            }}
          />
        </div>
        ...
        <div className="flex justify-between text-xs text-slate-400 mt-2">
          <span>{dateFromPosition(leftHandle).toLocaleDateString()}</span>
          <span>{dateFromPosition(rightHandle).toLocaleDateString()}</span>
        </div>
      </div>

      {/* Data summary */}
      <div className="text-xs text-slate-400 text-center pb-4">
        Mostrando {data.length.toLocaleString()} puntos de datos |
        Rango total: {dataBounds.minDate.toLocaleDateString()} - {dataBounds.maxDate.toLocaleDateString()}
      </div>
    </div>
  );
}