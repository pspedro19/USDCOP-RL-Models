...
import { TrendingUp, Download, Eye, EyeOff, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function EquityCurveChart({ data, isLoading = false, error = null, onRetry }: EquityCurveChartProps) {
  // ========== STATE ==========
      ...
      <Card className="bg-slate-900 border-cyan-500/20 shadow-xl">
        <CardContent className="flex items-center justify-center py-24">
          <div className="text-center">
            ...
            <p className="text-slate-300 font-mono text-lg">Loading Equity Curves...</p>
            <p className="text-slate-500 text-sm mt-2">Fetching performance data</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (error) {
    return (
      <Card className="bg-slate-900 border-red-500/30 shadow-xl">
        <CardContent className="py-12 text-center">
          <AlertCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
          <p className="text-red-400 font-mono text-lg mb-2">Error Loading Equity Curves</p>
  ...
  if (!data || data.length === 0) {
    return (
      <Card className="bg-slate-900 border-yellow-500/30 shadow-xl">
        <CardContent className="py-12 text-center">
          <TrendingUp className="h-12 w-12 text-yellow-500 mx-auto mb-4" />
          <p className="text-yellow-400 font-mono text-lg mb-2">No Equity Data Available</p>
          <p className="text-slate-400 text-sm mb-2">Execute DAGs: L5-Serving, L6-Backtest</p>
          <p className="text-slate-500 text-xs">Data will appear once the pipeline generates equity curves</p>
          {onRetry && (
            <Button
              onClick={onRetry}
