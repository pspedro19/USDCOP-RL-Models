...
export default function RealTimeChart() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        console.log('[RealTimeChart] Loading ALL historical data from 2020...');
        const endDate = new Date(); // Today
        const startDate = new Date('2020-01-01T00:00:00Z'); // From 2020
        ...
        console.error('[RealTimeChart] Error loading professional data:', error);
        setLoading(false);
      }
    };
  ...
  const handleAlignDataset = useCallback(async () => {
    try {
      ...
      console.error('[RealTimeChart] Error aligning professional dataset:', error);
      setLoading(false);
    }
  }, [currentPrice]);
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <div className="h-96 bg-[var(--bg-primary)] rounded-xl flex items-center justify-center">
          <div className="text-center text-[var(--text-secondary)]">
            <Activity className="h-8 w-8 mx-auto mb-4 animate-pulse" />
            <p className="text-lg font-medium">Loading Professional Chart...</p>
            <p className="text-sm mt-1">Initializing Bloomberg Terminal style</p>
          </div>
        </div>
      </div>
    );
