...
/**
 * BacktestTerminalPage — contenido compartido de /dashboard y /replay (BL-34)
 * ===========================================================================
 * GM re-skin (CTR-GM-UI-001): GmPageHeader único + selector de estrategia con dot
 * pulse + badge "OOS 2025 · BACKTEST". El contenido es ForecastingBacktestSection
 * (KPIs, candle+replay, equity, resumen, razones de salida, tabla, gates Voto 1).
 *
 * Dos variantes (una sola implementación — DRY):
 *  - `dashboard`: superficie de APROBACIÓN. Incluye el flujo Voto 2/2 intacto
 *    (admin-only, approval-gates.md) — los botones de aprobación viven SOLO aquí.
 *  - `replay`: superficie de INVESTIGACIÓN READ-ONLY (constitución / plan 00 §2).
 *    Misma sección con `readOnly`: cero botones de Vote-2 (ni para admin), nota
 *    explícita de solo-lectura y cifras del replay etiquetadas como PREVIEW.
 *    Remediación del rechazo Codex BL-34 ("/replay reexporta /dashboard y exhibe
 *    botones admin Aprobar/Rechazar").
 */

import { Suspense, useState, useEffect, useRef } from "react";
import { TerminalShell, type GmSection } from "@/components/gm/TerminalShell";
...
const DICT = defineGmDict({
  es: {
    ...
    replayTitle: 'Replay',
    replayKicker: 'Investigación · Solo lectura',
  ...
  en: {
    ...
    replayTitle: 'Replay',
    replayKicker: 'Research · Read-only',
...
// Page Content (shared)
// ============================================================================
export type BacktestVariant = 'dashboard' | 'replay';

function BacktestContent({ variant }: { variant: BacktestVariant }) {
  const t = useGmT(DICT);
  ...
  return (
    <TerminalShell active={(isReplay ? 'replay' : 'dashboard') as GmSection} width="wide">
      {/* Header único (prototipo: título + selector con dot pulse + badge OOS) */}
      <GmPageHeader
        kicker={isReplay ? t('replayKicker') : t('kicker')}
        title={isReplay ? t('replayTitle') : t('title')}
        actions={
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            {isReplay && <GmBadge tone="warn">{t('readOnlyBadge')}</GmBadge>}
      ...
      {/* ================================================================
          Forecasting Backtest (2025 OOS).
          - dashboard: + Approval — Vote 2/2 flow intacto (approval-gates.md)
          - replay:    readOnly — SIN superficie de Vote-2, cifras = PREVIEW
      ================================================================ */}
      <ForecastingBacktestSection
        controlledStrategyId={selectedStrategyId}
...
// Export
// ============================================================================
export function BacktestTerminalPage({ variant }: { variant: BacktestVariant }) {
  return (
    <Suspense fallback={<BacktestSkeleton />}>
      <BacktestContent variant={variant} />
    </Suspense>
  );
}
