...
/**
 * S3 · Trust bar — ONLY ○ PAPER (forward) metrics, straight from the published bundle via
...
export default function TrustBar() {
  ...
  const items: Array<{ label: string; value: string }> = [
    { label: 'en paper trading', value: `${stats.weeks_live ?? '—'} semanas` },
    {
  ...
  return (
    <section
      aria-label="Métricas de producción en papel (paper trading)"
    ...
    >
      <div className="max-w-6xl mx-auto px-4 py-4 flex flex-wrap items-center justify-center gap-x-8 gap-y-2">
        ...
        <span className="text-[11px] text-slate-500">
          señales publicadas antes del hecho · sin edición retroactiva · ejecución simulada (paper)
        </span>
      </div>
    </section>
  );
