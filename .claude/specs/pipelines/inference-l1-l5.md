...
> Created: 2026-02-12
>
> **Scope — RL track only (DEPRIORITIZED/PAUSED).** This L1→L5 flow and its `inference_ready_nrt`
> table serve the **RL PPO** model, which is deprioritized. `inference_ready_nrt` is **NOT** the live
> feed for the promoted production strategy. The production strategy is **H5 weekly**
> (`smart_simple_v11`): `core_l0_02_ohlcv_realtime` writes `usdcop_m5_ohlcv`, and
> `forecast_h5_l7_multiday_executor` reads that table **directly** via `_get_latest_bar()`
> (`SELECT … ORDER BY time DESC LIMIT 1`) — it never touches `inference_ready_nrt`.

> **Known issue (pre-existing, does NOT affect H5 production):** on a clean-slate cold boot the
> `usdcop-mlops-inference` container (:8003, RL inference — deprioritized) fails to start with
> `ModuleNotFoundError: No module named 'src'` (its PYTHONPATH / `src` mount is missing). Since the
> H5 production path reads `usdcop_m5_ohlcv` directly (not :8003), promotion/execution is unaffected.
> Fix belongs to that container's Dockerfile/compose `src` mount, not this pipeline.
...
> **RL-only table.** Written/read exclusively by the RL L1/L5 DAGs (deprioritized). The H5 production
> path does not use it — see the scope note at the top of this file.
...
> Corrects drift found by the 10-agent audit (see `../audit/AUDIT-2026-07-remediation.md` §A2).

- **A2-02 — verified DEAD, recommend deletion.** `l1_nrt_data_service.py` + `l5_nrt_inference_service.py` still exist in `src/services/` and are lazy-exported by `__init__.py`, but **no runtime code imports them** — the only references outside the package are *comments* in `l1_model_promotion.py` ("This replaces `L1NRTDataService.on_model_approved()`"). They are superseded by the DAG path and safe to delete (files + their `__init__.py` import blocks + `__all__` entries), keeping `BacktestFeatureBuilder`. Deletion left for operator approval (source removal); the "L1 is the only feature-computation layer" invariant already holds via the DAGs.
