...
>
> Read `architecture-overview.md` first (the as-built map). This spec turns that map into
> a repeatable procedure.
>
> Contract: CTR-ASSET-ONBOARD-001
> Version: 1.1.0
> Date: 2026-07-03 (status updated 2026-07-05)
> Status: PARTIALLY REALIZED — `AssetProfile` now EXISTS on disk
> (`src/contracts/asset_profile.py` + `config/assets/{usdcop,xauusd,btcusdt}.yaml`) and **Gold
> (XAU/USD) has been onboarded through Stage F** (data ingested, science, backtest+gates, published to
> the registry, visible + replayable on the web). **BTC/USDT is the 2nd worked case, at Stage-A
> onboarding**: AssetProfile (crypto/24-7/√365), crypto-native tables (migration 052), and onboarding
> tests A1/B1 have landed; data ingestion (Stage B, real extractors) and modeling are pending — full
> plan in `assets/btcusdt/` (SPEC-13 = the integration bridge). Prescriptive playbook below stands;
> for the **as-built** state of what is actually implemented today (ingestion,
> session/timezone/annualization, science, backend, frontend), see **`_asbuilt-implementation.md`**
> (CTR-ASSET-IMPL-001). Still proposed: RL env (SPEC-05/08), DAG factory, live execution, BTC data
> layer + backtest.
...
That crosses three difficulty tiers (Easy / Moderate / Hard, per `architecture-overview.md` §7).
...
Gates E1-E5 mirror the existing 5 approval gates (`approval-gates.md`) — the onboarding
...
| Concern | Spec |
|---------|------|
| As-built architecture map | `architecture-overview.md` |
| Dynamic registry so the new asset appears in the UI without frontend edits | `registry-lifecycle.md` |
| L0 OHLCV + macro governance (timezone, TwelveData quirks, multi-pair) | `data-governance.md` |
| Feature contract + norm stats + retrain | `training-l2-l3-l4.md`, `inference-l1-l5.md` |
| Strategy schemas + registry + signal contract | `strategy-contract.md` |
| Approval gates (mirror onboarding tests E1-E5) | `approval-gates.md` |
| Dashboard artifact schemas (per-asset folders) | `dashboard-integration.md` |
| Experiment discipline for tuning the new asset | `experiment-protocol.md`, `ssot-versioning.md` |
| H5 execution logic (inherited by per-asset strategies) | `h5-smart-simple.md` |
...
- Do NOT fork the feature contract while TS↔Python drift exists (`architecture-overview.md`
