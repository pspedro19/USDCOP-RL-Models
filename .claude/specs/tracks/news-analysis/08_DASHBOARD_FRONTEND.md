...
**Asset-aware hooks (2026-07):** the same hook module also exports asset-scoped variants
used by the selector (§9). All pass `?asset=<id>` through to the routes above:

```typescript
export function useAnalysisAssets()               // /api/analysis/assets → selector options
export function useAnalysisIndex(asset)           // /api/analysis/weeks?asset=
export function useWeeklyView(year, week, asset)  // /api/analysis/week/Y/W?asset=
export function useUpcomingEvents(asset)          // /api/analysis/calendar?asset=
```
...
|--------|------|--------|----------|
| GET | `/api/analysis/assets` | `analysis-assets.ts` (SSOT) | `AnalysisAsset[]` |
| GET | `/api/analysis/weeks?asset=` | `analysis_index.json` | `WeekListResponse` |
| GET | `/api/analysis/week/[year]/[week]?asset=` | `weekly_Y_WXX.json` | `WeekView` |
| GET | `/api/analysis/calendar?asset=` | `upcoming_events.json` | `UpcomingEvent[]` |
| POST | `/api/analysis/chat` | Azure OpenAI / DB (reads `body.asset`) | `ChatResponse` |

> **Asset-aware routing (2026-07):** all data routes accept `?asset=<id>` (default `usdcop`,
> backward compatible); chat reads `body.asset`. Server-side resolution via
> `lib/analysis-paths.ts` (`readAnalysisJson`) reads per-asset dir
> `public/data/analysis/<asset>/…` with legacy-root fallback for USD/COP. Path-safe:
> unknown/traversal ids collapse to `usdcop`. The new `GET /api/analysis/assets` route
> exposes the `ANALYSIS_ASSETS` SSOT and drives the selector dynamically. See §9.
...
## 9. Multi-Asset Asset Selector (2026-07)

The `/analysis` page now has a **dynamic asset selector**: **USD/COP**, **Gold** (`xauusd`),
**Bitcoin** (`btcusdt`). Switching asset shows that asset's per-week weekly + daily analysis.

```
+------------------------------------------------------------------+
| [ USD/COP ]  [ Gold ]  [ Bitcoin ]        ← AssetSelector (pills) |
+------------------------------------------------------------------+
| [< W08]  SEMANA 9  ·  ...  (per selected asset)                  |
+------------------------------------------------------------------+
```
...
| File | Role |
|------|------|
| `components/analysis/AssetSelector.tsx` | Renders one pill per asset from the SSOT |
| `lib/contracts/analysis-assets.ts` | SSOT: `ANALYSIS_ASSETS`, `resolveAnalysisAsset`, `isValidAnalysisAsset` |
| `app/api/analysis/assets/route.ts` | `GET /api/analysis/assets` — exposes SSOT, drives selector dynamically |
| `lib/analysis-paths.ts` | Server-side resolution `readAnalysisJson` — per-asset dir + legacy fallback, path-safe |

### State & Chat Sync

- `AnalysisPage.tsx` holds `selectedAsset` state; **resets the selected week on asset switch**
  and syncs the chat context.
- `stores/useAnalysisChatStore.ts` now carries `contextAsset`; the floating chat sends `asset`
  in its request body (consumed by `POST /api/analysis/chat`).

### Data Sources per Asset

| Asset | Generator | Path |
|-------|-----------|------|
| USD/COP | Macro LangGraph pipeline (unchanged, SDD-07) | legacy root `public/data/analysis/…` |
| Gold (`xauusd`) | `src/analysis/asset_analysis_generator.py` (real OHLCV/technicals/strategy/news) | `public/data/analysis/xauusd/…` |
| Bitcoin (`btcusdt`) | `src/analysis/asset_analysis_generator.py` (real OHLCV/technicals/strategy/news) | `public/data/analysis/btcusdt/…` |

> Backward compatible: routes default to `usdcop` when `?asset=` is absent, and unknown/traversal
> ids collapse to `usdcop` (path-safe). See §5 for the asset-aware routing note.
