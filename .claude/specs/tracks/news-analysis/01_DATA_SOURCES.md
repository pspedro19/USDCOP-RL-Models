...
## 7. Módulo de Noticias Multi-Activo (Gold / BTC) — Ports & Adapters

> **Añadido 2026-07-05.** Módulo **pluggable** que alimenta el análisis **por activo** (XAU/USD, BTC/USDT).
> Es **independiente** de los adapters de ingesta del news-engine de COP (`src/news_engine/ingestion/*`),
> que siguen siendo la fuente para USD/COP. El módulo multi-activo vive en `src/analysis/news_sources.py`.

### 7.1 Arquitectura (ports & adapters)

| Elemento | Rol |
|----------|-----|
| `NewsSource` (protocol) | **Port** — interfaz común que todo adapter implementa |
| `GoogleNewsSource` | **Adapter PRIMARIO** — Google News RSS |
| `GDELTSource` | **Adapter FALLBACK** — GDELT DOC 2.0 |
| `SOURCE_REGISTRY` | Mapa `name → class` para resolución dinámica |
| `build_news_sources(cfg)` | Factory: construye la lista de adapters desde config |
| `AssetNewsFetcher` | Fachada: itera fuentes según estrategia (`first_nonempty` por defecto \| `aggregate`) |

**Añadir una fuente** = escribir una clase adapter (que cumpla `NewsSource`) + registrarla en `SOURCE_REGISTRY`.
No requiere tocar el pipeline de análisis.

### 7.2 GoogleNewsSource (PRIMARIO)
...
| **Rate limit** | Ninguno |
| **Parser** | `feedparser` |
| **Histórico** | Sí — queries acotadas por fecha vía operadores `after:` / `before:` |
| **Cobertura** | Agrega Investing.com, CNBC, CoinDesk, Reuters, Yahoo Finance, etc. |
| **Query por activo** | `config/analysis/analysis_assets.yaml` → `assets.<id>.google_news_query` |

### 7.3 GDELTSource (FALLBACK)

| Atributo | Valor |
|----------|-------|
| **Base URL** | GDELT DOC 2.0 (público, sin key) |
| **Rate limit** | Estricto: **≤ 1 request / 5 s** → HTTP 429 al excederlo |
| **Mitigación** | Pacing global (`min_interval_s`, default 6) + backoff (`backoff_seconds`) |
| **Uso** | Solo si la fuente primaria no retorna artículos |

### 7.4 SSOT de configuración

Orden de fuentes y config por-fuente viven en `config/analysis/analysis_assets.yaml` bajo la clave `news:`:

```yaml
news:
  sources: ["google_news", "gdelt"]   # orden de intento
  strategy: first_nonempty            # first_nonempty | aggregate
  google_news:
    # knobs por-fuente
  gdelt:
    min_interval_s: 6
...
**Añadir un activo analizado** (futuro índice/futuro) = **UNA** entrada en `config/analysis/analysis_assets.yaml`
(+ su espejo TypeScript `usdcop-trading-dashboard/lib/contracts/analysis-assets.ts`).

### 7.5 Lección aprendida y resultado real

- GDELT hace **hard-429** de forma consistente; **Google News RSS** es la fuente histórica confiable → ahora es **primaria**.
- Resultado real: **~40 artículos/semana por activo**, **1,080 por activo** en 27 semanas (Gold + BTC).
