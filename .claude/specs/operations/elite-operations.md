...
>
> **SSOT ownership**: this file is authoritative for the **DAG schedule / collision-free timeline**.
> **Freshness thresholds + recovery procedures** are owned by `../../rules/data-freshness.md` — refer
> there, do not re-tabulate thresholds here.
...
Migration:  049_regime_gate_columns.sql   # (048 is reconciliation_tables; audit A1-07)
```
...
> **H1 track PAUSED (audit A3-01)**: the 5 `forecast_h1_*` DAGs are created with
> `is_paused_upon_creation=True` and do NOT auto-run (no auto-run on DB reset). Their timings below
> document *intended* schedule only — production today = **H5 weekly**.
> **Seed backup** runs `0 20 * * 1-5` (15:00 COT, **Mon-Fri only — no weekend backup**).
...
06:00  H1-L3 Training (PAUSED, audit A3-01 → intended: 9 models)
06:30  H5-L3 Training (data freshness gate → Ridge + BR + XGB)
06:45  asset_xauusd_pipeline_weekly (Gold DS-cycle: l0_ingest → l0b_export_chart_ohlcv → l4_backtest_publish → l5_weekly_forecast → l6_verify_registry)
07:00  asset_btcusdt_pipeline_weekly (BTC DS-cycle: l0_ingest → l0b_export_chart_ohlcv → l4_backtest_publish → l5_weekly_forecast → l6_verify_registry)
       (l0b = backtest-chart JSON for /dashboard; l5 = whole-year weekly inference for /forecasting; both graceful, run inside the existing slot → no new collision)
07:30  Training finishes
       (Seed Backup does NOT run Sunday — Mon-Fri only, `0 20 * * 1-5`)
```
...
```
forecast_h5_l4_backtest_promotion   Manual — OOS backtest + gates (Vote 1/2); ends with
                                    register_bundle registry exit-gate (CTR-STRAT-REGISTRY-001 §6.2)
forecast_h5_l4b_production_deploy   Triggered by the dashboard deploy API (Airflow REST, post-Vote-2):
                                    guard APPROVED → manifest command (--phase production --seed-db)
                                    → validate summary.json → register_bundle. Mirrors progress into
                                    deploy_status.json (runner: airflow). Added 2026-07-07.
```

> **Airflow bootstrap note (2026-07-07):** `timescale_conn` is provided as the env var
> `AIRFLOW_CONN_TIMESCALE_CONN` in docker-compose (scheduler + webserver) so it survives
> metadata-DB resets — the missing connection used to silently kill the macro backfill's
> health_check. The REST API has `basic_auth` enabled for the dashboard deploy trigger.
