...
| `/analysis` | **Multi-asset selector** (USD/COP · Gold · Bitcoin) + week selector, weekly summary, technical card, scenarios, regime, signal cards, macro charts, daily timeline, calendar, chat widget | `/api/analysis/weeks`, `/week/{y}/{w}`, `/calendar`, `/chat` (asset-namespaced) | selector now drives asset; COP keeps macro fields |
