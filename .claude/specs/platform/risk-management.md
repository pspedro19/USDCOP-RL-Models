...
>
> **Subsystem landscape (as-built, reconciled 2026-07-07 — audit A7-02/03/04):**
> - **Chain of Responsibility** (`src/risk/checks/`): 9 modular pre-signal checks. Its
>   production consumer is `services/mlops/risk_manager.py` (below).
> - **MLOps RiskManager** (`services/mlops/risk_manager.py`) — **the 4th subsystem the old spec
>   never mentioned (A7-04)**: Redis-persisted wrapper around RiskCheckChain; this is what the
>   mlops/RL inference path actually calls. The legacy `src/risk/risk_manager.py` duplicates it
>   and remains only as a library fallback.
> - **Enforcement Layer** (`src/trading/risk_enforcer.py`): 7 pluggable rules at execution time.
> - **PreTradeGate** (`services/signalbridge_api/app/services/pretrade.py`): the LAST gate before
>   any exchange order — global trading_mode (PAPER default simulates), per-user kill switch,
>   notional/day caps; fail-safe BLOCK. Multi-tenant kills write to the append-only `audit_log`.
> - **Command Pattern** (`src/risk/commands.py`): **DEPRECATED / NOT WIRED (A7-02)** — 974 lines
>   of undo/redo command framework imported only by its test; no DAG/service/API triggers it.
>   Decision 2026-07-07: kept as dormant library, no longer presented as live. Do not build new
>   consumers on it without an ADR.
> - **Kill-switch audit (A7-03)**: the live audit surface is the append-only **`audit_log`**
>   (migration 055) — tenant kill on/off, global kill, fan-out grants/denies all write rows
>   (verified in the fan-out E2E). The legacy `audit.kill_switch_audit` table (migration 014)
>   was never called by any code and is **superseded** — kept for schema stability, not used.
>
...
|-------|-------|------|-----------|----------------|
| 0 | HOLD/Exit Signal | `hold_signal_check.py` | Signal ∈ {HOLD, CLOSE, FLAT, EXIT} | Short-circuit (approved). **Audit A7-01 (fixed 2026-07-07): position-CLOSING signals also pass through — a closing order must never be blocked by entry-oriented checks** (parity with Enforcer/Manager, which already exempted exits) |
...
- `h5-smart-simple.md` -- H5 weekly guardrails (circuit breaker, long insistence, rolling DA)
- `mlops-lifecycle.md` -- Pipeline guardrails overview
- `inference-l1-l5.md` -- L5 inference where risk checks apply at runtime
- `strategy-contract.md` -- Universal signal record consumed by risk checks
- `data-freshness.md` -- Data quality gates (complementary to risk gates)
...
> Corrects drift found by the 10-agent audit (see `../audit/AUDIT-2026-07-remediation.md` §A7).

- **There are 4 overlapping risk subsystems, not 3.** In addition to `src/risk/` (`RiskCheckChain` via `checks/` + `risk_manager.py`), there is a separate `src/trading/risk_enforcer.py` (`RiskEnforcer`, rule-based). The two are not unified (A7-04).
- **The Command Pattern (`src/risk/commands.py`) and the kill-switch audit table are present but largely unwired** — document as scaffolding, not active enforcement, until integrated (A7-02/03).
