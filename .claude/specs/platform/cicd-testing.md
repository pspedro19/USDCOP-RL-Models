...
|-------|-------------------|
| Deprecated imports | `scripts/ops/migrate_imports.py --check` |
...
| **Database** | `db-migrate` | `python scripts/ops/db_migrate.py` | Run pending migrations |
| | `db-status` | `python scripts/ops/db_migrate.py --status` | Check migration status |
| | `db-validate` | `python scripts/ops/db_migrate.py --validate` | Validate required tables |
...
### System Validation Harnesses (manual, against a running stack)

Non-CI orchestrators for whole-system acceptance (live `docker compose` stack, not pytest):

| Harness | Script | Destructive? | What it does |
|---------|--------|--------------|--------------|
| E2E suite | `scripts/validation/run_e2e_suite.py` | **No** | 8 isolated checks: stack health, registry integrity (3 assets / 8 strategies, no Inf/NaN), asset DAGs loadable, per-asset bundles on disk, feature-backup manifest, promotion/approval state, `/analysis` endpoints, H5 live path. Emits `results/e2e/report.json` + appends `results/e2e/report.md` |
| Teardown/restore | `scripts/validation/teardown_restore_test.sh` | **Yes** (`down -v`) | Clean-slate acceptance: `CYCLES` cold-boot cycles (`down -v`→`up -d`→auto-restore), asserts OHLCV+macro+news restored and web serves 3 assets / 8 strategies. Proven 3/3 deterministic green. Run LAST, after promotion populated the DB + feature backup |
...
- `mlops-lifecycle.md` -- 8-stage lifecycle (CI/CD integrates at Stage 2 train + Stage 6 deploy)
- `approval-gates.md` -- Approval gates (CI validates gates, CD deploys after approval)
- `strategy-contract.md` -- Strategy contracts validated by `contracts-check.yml`
...
- `data-freshness.md` -- Data gates run in Airflow, drift monitored by `drift-check.yml`
