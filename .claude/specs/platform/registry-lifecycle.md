...
> from the UI.
>
> This spec EVOLVES `strategy-contract.md` (StrategyManifest/Registry/Selector were single-asset,
> single-summary) into a scalable, registry-driven design. Read `architecture-overview.md`
> for the as-built state and `_onboarding-playbook.md` for the AssetProfile it consumes.
>
...
> | Immutable versioned publisher (§5) | `strategy_manifest.py::BundlePublisher` — content-addressed `(strategy_id, version, year)`, never overwrites. **`phase="production"` (audit A4-01, 2026-07-07): the LIVE year writes MUTABLE `strategies/<sid>/production/{summary,trades}.json` + sets the `manifest.production` pointer — never routed through the immutable path** | ✅ |
> | Pipeline wiring | `scripts/pipeline/train_and_export_smart_simple.py::publish_versioned_bundle()` — called AFTER legacy writes, guarded; the production call passes `phase="production"` | ✅ |
> | DAG EXIT contract (§6.2) | `airflow/dags/utils/register_bundle.py::register_strategy_bundle()` — **WIRED (audit A4-02, 2026-07-07) as the terminal task of `forecast_h5_l4_backtest_promotion` AND `forecast_h5_l4b_production_deploy`** (previously defined but never called by any DAG) | ✅ |
> | TS manifest contract (audit A4-03) | `lib/contracts/strategy-manifest.contract.ts` — field-for-field mirror of the Python dataclasses; `/api/registry`, `/api/registry/promote`, `/api/strategies/[id]/manifest` fully typed | ✅ |
...
> | DS-cycle asset DAG factory (ingest→backtest+publish→verify) | `airflow/dags/asset_pipeline_factory.py` + SSOT `config/assets/pipelines.yaml` (CTR-ASSET-PIPELINE-001) — emits `asset_xauusd/btcusdt_pipeline_weekly`; `l4_backtest_publish` runs the publisher, `l6_verify_registry` asserts registry.json has the asset + its 3 bundles | ✅ |
| Full forecast-pipeline DAG factory (5-DAG train→signal→exec→monitor gen) | §6.3 | ⏳ PROPOSED |
...
**Current blockers** (from `architecture-overview.md`):
...
```jsonc
{
  ...
  "asset_id": "xauusd",                          // FK → AssetProfile (_onboarding-playbook)
...
- JSON safety per `strategy-contract.md` (`safe_json_dump`, no Infinity/NaN).
...
| Data freshness | `validate_training_data_freshness()` gate (BLOCKING) per `data-freshness.md` |
...
> **Already shipped (2026-07-05) — a simpler DS-cycle variant of this factory:**
> `airflow/dags/asset_pipeline_factory.py` reads SSOT `config/assets/pipelines.yaml` and emits ONE
> `asset_<id>_pipeline_weekly` DAG per enabled asset (Gold Sun 01:45 COT, BTC Sun 02:00 COT) whose
> tasks are `l0_ingest` (graceful — never blocks the science stage) → `l4_backtest_publish` (runs the
> per-asset runner = the §5 `BundlePublisher` EXIT contract) → `l6_verify_registry` (asserts the asset +
> its strategy bundles are in `registry.json`). This gives Gold/BTC config-driven **publishing** DAGs
> today (add an asset = one yaml entry, no DAG code). The full train→signal→execute→monitor generation
> above (§6.3) is still the proposed scale-out for tradeable pipelines.
...
- `signals` parquet (`UniversalSignalRecord[]`, `strategy-contract.md`),
...
Every hardcoded value called out in `architecture-overview.md` §6 (`symbol="USDCOP"`,
...
R4 also closes the drift risk from `architecture-overview.md` §5.3 by making the UI symbol
manifest-driven.
...
6. ⏳ **PROPOSED — Reconcile feature-contract drift** (`architecture-overview.md` §5.3)
...
| Concern | Spec |
|---------|------|
| As-built state + hardcoded coupling | `architecture-overview.md` |
| AssetProfile (asset params consumed by ENTRY rules) | `_onboarding-playbook.md` |
| Trade/summary/gate schemas + UniversalSignalRecord | `strategy-contract.md` |
| Approval gates (Vote 1 computed in register_bundle) | `approval-gates.md` |
| Dashboard artifact schemas / page flows | `dashboard-integration.md` |
| Frozen config = model_version | `ssot-versioning.md`, `experiment-protocol.md` |
| Data-freshness ENTRY gate | `data-freshness.md` |
| Replay engine + SSE | `execution-bridge.md`, `backtest.contract.ts` |
...
> Corrects drift found by the 10-agent audit (see `../audit/AUDIT-2026-07-remediation.md` §A4).

- **`register_bundle` is NOT called from any DAG exit gate.** `airflow/dags/utils/register_bundle.py` exists as a standalone util invoked manually; the "register on promote" step is not automated. The ✅ status on that step is aspirational (A4-02).
- **Replay signals are persisted as `.json`, not `.parquet`** as some passages state (A4-12).
- **No production pointer is written** on promote — the headline/active-strategy sync is manual (A4-01/05).

### As-built today (2026-07-05) — the registry is live and multi-asset

The abstract contracts above are **realized and populated**. `registry.json` currently indexes
**3 assets · 8 strategies**, all published via `BundlePublisher.publish(...)` (immutable, additive):
...
versions) and **a new strategy branch** (`smart_simple_aggr`), plus **three full assets** rendering
with their own `chart_symbol` (USDCOP / XAUUSD / BTCUSDT). Adding BTC required **zero frontend code** —
publishing the bundles was the whole integration. The science behind Gold/BTC strategies (variables,
...
the manifest (`model_versions[].active` + `production.model_version`) — it writes **no** production DB
pointer and triggers **no** deploy. Only `smart_simple_v11` has `has_production: true`.

**Per-asset DS-cycle DAGs (new, 2026-07-05).** Gold/BTC now have their own weekly pipeline DAGs —
`asset_xauusd_pipeline_weekly` and `asset_btcusdt_pipeline_weekly` — **generated** by
`airflow/dags/asset_pipeline_factory.py` from the SSOT `config/assets/pipelines.yaml`. Each runs the
DS-cycle `l0_ingest → l4_backtest_publish → l6_verify_registry` (ingest bars → backtest + publish
immutable bundle → verify the registry entry). Schedules: Gold `45 6 * * 0` (Sun 01:45 COT),
BTC `0 7 * * 0` (Sun 02:00 COT). This realizes the factory pattern of §6.3 for the daily science
stacks (a lighter 3-task DS-cycle, not the H1/H5 5-DAG shape).
