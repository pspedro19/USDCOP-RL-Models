...
| `market_ohlcv_{5m,1h,4h,daily}_wide` | views wide | clave `bar_start_utc`; por activo: OHLCV + `source`/`bar_origin`/`is_session_bar`/`staleness_seconds` (vivo, desde `now()`) + `status` ∈ `closed·ok·partial·missing·pending·off_session·incoherent·no_native_data`. **`closed`≠`missing`**: 2026-07-20 lee `usdcop=closed, btcusdt=ok`; hoy-sin-barra-diaria = `pending`, nunca falso `missing`; barra presente con calendario cerrado (COP offshore en festivo, ~12 d/año) = `off_session`; pre-primera-barra del activo = `no_native_data`. Calendario NYSE = federal − Columbus/Veterans + Viernes Santo + quirks 2021/Carter-2025 verificados contra barras reales. SPX sin intradía → `no_native_data` (no se fabrica microestructura). La diaria incluye las 17 macros CLEAN en forma as-of y `_t1` (única forma legal para modelos); MXN/CLP excluidas (corrupción ×10⁴, guard en CI) |
...
**Vida y calidad (remediación 2026-07-22)**: `l0_multiframe_catchup` (horario, hh:20) mantiene vivas TODAS las capas — M5 (XAU/COP-gaps/MXN/BRL vía TD, BTC vía Binance), 1h/4h/1month nativos, continuidad diaria (incl. SPY/BTC) y REFRESH de matviews; el writer diario deduplica **por fecha UTC** (feeds estampan horas distintas). Macro mensual: `scripts/ops/refresh_monthly_macro.py` (FRED + SUAMECA REST + DANE Excel + Fedesarrollo PDF; scrapers restaurados de d47253e^ y ahora tracked) — IPC vía DANE `anex-IPC-Indices` (empalme verificado exacto), RESINT vía REST 15051; **solo TOT queda pendiente de Selenium**. MXN/CLP diarias reparadas (×10⁴; TwelveData promovida a primaria en el SSOT — investing 403 + coma-decimal) y **reincorporadas a la wide diaria** (as-of + `_t1`). `available_at` con DEFAULT now() en las 3 tablas físicas. Postgres: `shm_size: 1gb` + `max_locks_per_transaction=4096` (la historia profunda multiplicó chunks).

Guardas: `tests/regression/test_wide_views.py` (estructurales siempre; DB-backed con stack arriba) + `test_data_quality_floor.py` (**ratchet**: coherencia=0 incoherentes, series mensuales sin apagarse más allá de su lag declarado, MXN/CLP sin re-corromperse, wide-no-inventa, completitud COP M5 ≥96%, PIT universal post-cableado, cero fechas diarias duplicadas) + `test_macro_clean_fx_scale.py` (ya duro, xfail levantado).
