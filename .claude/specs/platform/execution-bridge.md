...
> **H5 production real-time feed**: the executor reads the latest bar **directly** from
> `usdcop_m5_ohlcv` (written by `core_l0_02_ohlcv_realtime`) via `_get_latest_bar()`
> (`SELECT … ORDER BY time DESC LIMIT 1`). It does **not** use `inference_ready_nrt` — that table
> is RL-only (deprioritized). See `pipelines/inference-l1-l5.md`.
...
### PreTradeGate — enforcement at the order path (AS-BUILT 2026-07-06, OLA-3 S1-S3)

The audit found `ExecutionService.execute_order` went decrypt→adapter→`place_order` with
**no risk checks, no kill switch and no mode gate**. That hole is now closed by
`app/services/pretrade.py::PreTradeGate`, invoked FIRST in `execute_order`:

| Check | Source | Outcome |
|---|---|---|
| Global mode | `settings.trading_mode` | KILLED/DISABLED→**BLOCK** · PAPER/SHADOW→**SIMULATE** (recorded paper fill, never sent) · STAGING→testnet creds only · LIVE→continue |
| User kill switch / opt-in | `sb_trading_configs.trading_enabled` (default **False**) | no row / disabled → BLOCK (safe-by-default) |
| Symbol policy | `allowed_symbols` / `blocked_symbols` | outside policy → BLOCK |
| Notional cap | `user_risk_limits.max_position_size_usd` (else `settings.max_position_size_usd`) | over cap → BLOCK |
| Daily trade count | `sb_trading_configs.max_daily_trades` | reached → BLOCK |
| **Fail-safe** | any gate error | **BLOCK** (never fail-open with real money) |

Sizing (S1): orders are quantized with CCXT `amount_to_precision` via
`ExchangeAdapter.quantize_amount` (base adapter, DRY) — fixes the MEXC TICK_SIZE precision
bug where `10**-precision.amount` yielded qty=0 for BTC. `MEXCAdapter.get_symbol_info`
now handles both precision modes. Tests: `tests/test_pretrade_gate.py` (8, in-container).
...
| Spec | Relationship |
|------|-------------|
| `strategy-contract.md` | UniversalSignalRecord, execution strategies (WeeklyTPHSExecution, DailyTrailingStopExecution) |
| `mlops-lifecycle.md` | H1-L7 and H5-L7 DAG schedules, monitoring stage |
| `h5-smart-simple.md` | H5 executor architecture, TP/HS config, subtrade tracking |
| `approval-gates.md` | Strategy approval before production deploy |
| `inference-l1-l5.md` | RL L5 signal generation (SignalBridge consumer) |
| `dashboard-integration.md` | Dashboard `/execution` page inventory |
...
> Corrects drift found by the 10-agent audit (see `../audit/AUDIT-2026-07-remediation.md` §A6).

- **"The Gap" is partially closed.** `airflow/dags/utils/signalbridge_client.py` exists and the H5/H1 L7 executors call `place_order_via_bridge(...)` on **entry** when not in paper mode. The passages describing the DAG↔bridge connection as entirely missing are obsolete.
- **Entry-only forwarding (A6-01).** Exit/TP/HS/close events are still NOT forwarded to the OMS. As of this branch, both executors **hard-block a non-paper entry** unless `H{5,1}_ALLOW_NONPAPER_UNSAFE=true`, preventing orphaned exchange positions until exit-forwarding is built.
- **Executor surface is consolidated** into the shared client module rather than the multiple per-file adapters the older text implies (A6-04/08).

---

## MEXC spot paper copy-trade POC — PASS (2026-07-06)

End-to-end verification that a published signal becomes a per-user copy allocation and is
SIMULATED (never sent) in the default PAPER mode. Spot-only (`defaultType: spot`), real MEXC
connectivity, no real orders.

| Step | Result |
|---|---|
| MEXC credentials (read-only auth) | valid = True |
| Live BTC/USDT last price | 63,301.80 (real) |
| Sizing $200 → `quantize_amount` | 0.00315946 BTC (TICK_SIZE mode — no zero-out; audit fix holds) |
| Copy allocation (user cap $5,000) | $200 → correct qty |
| `PreTradeGate.check` (mode=PAPER) | **SIMULATE** — "order validated but not sent" |
| Order reaches exchange? | **NO** (simulated paper fill) |

Safety posture (unchanged): `trading_mode=PAPER` globally → SIMULATE; `SIGNALBRIDGE_DEV_MODE=false`;
per-user kill-switch default OFF; notional ceiling $5,000; withdraw-enabled keys rejected at
registration. **Real orders remain deferred** (require LIVE mode + funded account + explicit
confirmation + SFC legal gate for third-party money). Reproduce: PreTradeGate short-circuits to
SIMULATE in PAPER before any DB/user check, so the gate is fail-safe.

---

## S4 multi-tenant fan-out — WIRED + verified (2026-07-07)

The signal→tenants fan-out (CTR-RBAC-001 R5, S4) is now a real, audited path:

- **Endpoint**: `POST /api/tenant/system/fan-out` (admin-only; non-admin → 403 + `fanout_denied`
  audit row; success → `fanout` audit row). Invokes `tenant.py::fan_out_signal()` — previously
  dead code with zero callers.
- **Eligibility**: `user_risk_limits_v2.kill_switch = FALSE AND max_notional_usd > 0` JOIN
  `user_exchange_keys.status = 'verified'`. Per-user allocation = `min(signal_notional, user cap)`.
  Each row lands as a **PENDING paper** `user_executions` record — PreTradeGate still gates any
  real order (paper-first).
- **Pipeline integration**: `forecast_h5_l5_vol_targeting.py::persist_leverage` calls
  `utils/signalbridge_client.py::fan_out_signal_to_tenants()` AFTER the signal is final
  (leverage + regime gate + stops) and only when `skip_trade = false`. Best-effort by contract:
  failures log and never block the DAG; without `SIGNALBRIDGE_SERVICE_TOKEN` or
  `SIGNALBRIDGE_ADMIN_EMAIL/PASSWORD` env it skips gracefully. Notional =
  `FANOUT_BASE_NOTIONAL_USD` (default 10000) × adjusted_leverage; signal_id `h5_<date>`.
- **E2E** (`scripts/validation/signalbridge_fanout_e2e.sh`, 11/11): 2 paper tenants with
  different caps → fan-out honours per-user caps (50 vs capped 10 qty), rows PENDING, cascade
  kill excludes the killed user on re-fan, subscriber → 403 + audit. Self-cleaning.
