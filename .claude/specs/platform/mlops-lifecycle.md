...
python scripts/data/build_unified_fx_seed.py          # Rebuild FX seeds from raw sources
...
see `dashboard-integration.md` § Forecasting Page Data Contract.
...
See `h5-smart-simple.md` § Promotion Gates for full threshold table.
...
| 2a | `python scripts/pipeline/generate_weekly_forecasts.py` | Train H1 |
| 2b | `python scripts/pipeline/train_and_export_smart_simple.py --phase backtest` | Train H5 |
...
python scripts/pipeline/train_and_export_smart_simple.py --phase both --no-png
```
...
| `training-l2-l3-l4.md` | RL training pipeline (deprioritized) |
