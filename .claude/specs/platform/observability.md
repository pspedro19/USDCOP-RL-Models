...
- `data-freshness.md` -- Data freshness thresholds and automated recovery
- `backup-recovery.md` -- Backup architecture and disaster recovery
- `elite-operations.md` -- Operational rulebook, schedule coordination, monitoring checklist
- `data-governance.md` -- L0 OHLCV + macro pipeline (data that alerts monitor)
- `inference-l1-l5.md` -- Inference pipeline (model alerts)
...
> Corrects drift found by the 10-agent audit (see `../audit/AUDIT-2026-07-remediation.md` §A9).

- **Jaeger / OpenTelemetry IS instrumented** — `services/common/tracing.py`, `src/shared/tracing/otel_setup.py`, and the `inference_api` + `signalbridge_api` `main.py` wire OTel. The "0 services instrumented" statement (and the matching note in `CLAUDE.md`) is outdated.
- **MLflow L3 logging IS wired** — the H5-L3 training DAG calls `_mlflow_safe_start_run(...)` (non-blocking). MLflow is no longer "never invoked by DAGs".
- **Alerting is still a no-op by default** — AlertManager rules load but the Slack webhook is empty unless `SLACK_WEBHOOK_URL` is set (A9-07).
- **Some Prometheus scrape jobs target services not present in `compact`** — treat the scrape table as full-enterprise; prune dead jobs for compact (A9-06).
