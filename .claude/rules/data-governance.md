...
| 5 | `core_l0_05_seed_backup` | `l0_seed_backup.py` | DB → parquet backup for startup restore | Mon-Fri `0 20 * * 1-5` (15:00 COT) | OHLCV + macro |

### Backup Strategy
Two-tier backup system:
- **Automated weekdays** (#5): `l0_seed_backup` dumps OHLCV + macro tables to `data/backups/seeds/*.parquet` every weekday (Mon-Fri, `0 20 * * 1-5` = 15:00 COT) after market close — **no weekend backup**. These are the **freshest** backups, read first by init-scripts on `docker-compose up`.
...
- **Feature-data backup** (#5, added 2026-07-05): the same DAG also dumps 12 **derived** tables (news, analysis, H5 production, `asset_daily_ohlcv`, macro monthly/quarterly) to `data/backups/features/*.parquet` + `feature_backup_manifest.json` via `scripts/ops/backup/feature_data_backup.py` (CTR-L0-FEATURE-BACKUP-001). Best-effort task — never blocks the critical OHLCV/macro path. On cold boot the `data-seeder` container runs `--mode restore` (empty-table-only, never clobbers populated tables).
- **Restore priority**: Daily backup parquet (if exists) → Git LFS seed parquet → MinIO → legacy CSV.
- Restore is handled by init-scripts (`04-data-seeding.py`, `04-seed-from-minio.py`) on container startup; derived/feature tables are restored by the `data-seeder` (`feature_data_backup.py --mode restore`).
...
**For USD/COP (and the session-bounded FX pairs): no exceptions.** Every timestamp in every
seed file, parquet, and DB row must be in `America/Bogota` timezone. The session window is
**8:00-12:55 COT, Monday-Friday**.
...
> **Multi-asset carve-out (audit A1-01):** non-COP assets whose market is not COP-session-bounded
> (e.g. XAU/USD, migration `051_asset_daily_ohlcv.sql` + `scripts/ingest_asset_ohlcv.py`) store
> **instant-based `TIMESTAMPTZ`** (5-min at the UTC instant; daily at NY-close UTC), NOT COT-localized.
> The COT rule above is authoritative for COP; per-asset session/timezone lives in
> `../specs/assets/_asbuilt-implementation.md`.
>
> **Daily-bar date anchoring (CTR-DQ-OHLCV-001, learned the hard way 2026-07).** When resampling a daily
> bar to a session-close instant (e.g. XAU/USD 17:00 ET), anchor the **trading date in UTC space** —
> `ts.dt.tz_convert("UTC").dt.normalize()` — *then* apply the close-time offset and localize to the close
> tz. **NEVER `ts.dt.tz_convert("America/New_York").dt.normalize()` on a 00:00-UTC stamp**: 00:00 UTC is
> 19:00/20:00 the *previous* ET day, so `normalize()` shifts every bar back one calendar day (this is the
> Gold "Sunday pile-up" bug; root cause was `ingest_asset_ohlcv.py::_daily_to_nyclose`). Every ingest now
> runs the shared **OHLCV validator** (`src/data_quality/ohlcv_validators.py`) as a hard gate before
> writing a seed — it raises on any bar whose weekday ∉ `AssetProfile.session.days`, plus bars/period,
> calendar-gap, tz-of-close, dup, NaN and OHLC-integrity checks. Reuses the `build_unified_fx_seed.py`
> tz-strip→`tz_localize(true-tz)`→`tz_convert` discipline (below).
...
#### Regeneration
```bash
python scripts/data/build_unified_fx_seed.py
```
...
- **Schedule**: `0 20 * * 1-5` (20:00 UTC = 15:00 COT, Mon-Fri only — no weekend backup, after market close)
- **Purpose**: Full table dump of OHLCV + macro to parquet for startup restore
- **Output**: `data/backups/seeds/{usdcop_m5_ohlcv_backup,macro_indicators_daily_backup}.parquet` + `backup_manifest.json`; plus **feature data** to `data/backups/features/*.parquet` + `feature_backup_manifest.json` (news / analysis / `forecast_h5_*` / `asset_daily_ohlcv` / macro monthly+quarterly — 12 tables)
...
- **Contract**: CTR-L0-SEED-BACKUP-001 (+ CTR-L0-FEATURE-BACKUP-001 for the feature dump)
- Flow: `health_check -> export_ohlcv_backup -> refresh_daily_ohlcv_seed -> export_macro_backup -> export_feature_data_backup -> write_manifest -> validate_backups -> upload_to_minio` (feature backup + MinIO upload are best-effort; never fail the DAG)
...
- Do NOT `tz_convert(...→ET).normalize()` a 00:00-UTC daily bar to pick its date -- anchor the trading date in UTC first (shifts the day otherwise; the Gold Sunday-pile-up bug, CTR-DQ-OHLCV-001)
- Do NOT write an OHLCV seed that fails `src/data_quality/ohlcv_validators.py` -- off-session weekday bars are a hard ERROR; use `--no-validate` only as a logged emergency escape
