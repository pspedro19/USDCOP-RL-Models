...
Registry today: **3 assets · 8 strategies** (published bundles in `public/data/strategies/`, indexed by
`registry.json`). Cross-asset caveat: metrics are annualized per-asset (never compare across assets on
one clock — see `assets/_strategy-science.md §6`).

| Strategy ID | Name | Asset | Type | Timeframe | Status | Pipeline Script |
|-------------|------|-------|------|-----------|--------|-----------------|
| `smart_simple_v11` | Smart Simple v2.0.0 | usdcop | ml_supervised | weekly | **PRODUCTION** | `scripts/pipeline/train_and_export_smart_simple.py` |
| `smart_simple_aggr` | Smart Simple Aggressive | usdcop | ml_supervised | weekly | experimental (A/B) | `scripts/pipeline/train_and_export_smart_simple.py` |
| `forecast_vt_trailing` | Forecast + VT + Trail | usdcop | ml_supervised | daily | **PAUSED** | `scripts/pipeline/generate_weekly_forecasts.py` |
| `rl_v215b` | RL V21.5b | usdcop | rl_ppo | intraday_5m | DEPRIORITIZED | `scripts/pipeline/run_ssot_pipeline.py` |
| `gold_long_only_b1` | Gold · HODL vol-targeted (B1) | xauusd | rule_based | daily | experimental | `scripts/run_gold_pipeline.py` |
| `gold_trend_b2` | Gold · Trend-follower (B2) | xauusd | rule_based | daily | experimental (PROMOTE) | `scripts/run_gold_pipeline.py` |
| `gold_regime_gated_v1` | Gold · Regime-gated | xauusd | hybrid | daily | experimental | `scripts/run_gold_pipeline.py` |
| `btc_trend_b2` | Bitcoin · Trend-follower (B2) | btcusdt | rule_based | daily | experimental (PROMOTE) | `scripts/pipeline/run_btc_pipeline.py` |
| `btc_hodl_b1` | Bitcoin · HODL vol-targeted (B1) | btcusdt | rule_based | daily | experimental | `scripts/pipeline/run_btc_pipeline.py` |
| `btc_exposure_s3` | Bitcoin · Regime-gated exposure (S3) | btcusdt | hybrid | daily | experimental | `scripts/pipeline/run_btc_pipeline.py` |

> Gold/BTC are **rule-based daily science stacks** (`src/gold_rl/`, `src/btc_strategy/`) — see
> `assets/_strategy-science.md` for how each is built (intent × vol-targeting × regime), the variables,
> anti-leakage, evaluation, and per-asset results. They reach the dashboard purely by publishing a
> bundle to the registry (no frontend change) — see `../specs/platform/registry-lifecycle.md`.
...
# Compare strategies
python scripts/pipeline/replay_backtest_universal.py --compare smart_simple_v11,forecast_vt_trailing --year 2025
```
