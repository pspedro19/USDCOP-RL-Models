...
> Date: 2026-03-12
>
> **SSOT ownership**: this file is authoritative for **freshness thresholds + recovery procedures**.
> The **DAG schedule / collision-free timeline** is owned by `../specs/operations/elite-operations.md`
> — refer there, do not re-tabulate schedules here.
...
> **OHLCV structural quality gate (CTR-DQ-OHLCV-001, not a staleness check):** beyond freshness, every
> asset OHLCV seed must pass `src/data_quality/ohlcv_validators.py::validate_ohlcv_seed` at ingest —
> weekday coverage vs `AssetProfile.session.days`, bars/period, calendar gaps, tz-of-close, dup/NaN/OHLC
> integrity. Off-session weekday bars are a hard **ERROR** (the seed is not written). This is the gate
> that would have caught the Gold daily day-shift bug; wired into `ingest_asset_ohlcv.py` +
> `ingest_btc_ohlcv.py`. See `data-governance.md` (daily-bar date anchoring).
...
> **Fixed 2026-07-07 (3 stacked bugs kept this recovery from ever working in the container):**
> (1) `timescale_conn` was missing → now durable via `AIRFLOW_CONN_TIMESCALE_CONN` env in
> docker-compose (survives metadata-DB resets); (2) container-blind `PROJECT_ROOT` + sys.path
> shadowing in `l0_macro_backfill.py` (fixed layout-aware + append-only sys.path); (3) the regen
> task queried nonexistent short column names and wrote the wrong shape — rewritten to the CLEAN
> contract (UPPERCASE SSOT names + DatetimeIndex + TPM legacy D→M rename). Verified: parquet
> fresh to run date, 18 cols. If `/analysis` macro charts show "Sin datos", the CLEAN parquet is
> stale — run this recovery (or patch published weeks via
...
# 3. Run pending migrations (on cold boot init-scripts/26-restore-features.sh globs 043-054 automatically)
...
# 3b. Restore derived/feature data (news, analysis, H5, asset_daily) into empty tables.
#     On a clean-slate cold boot the data-seeder container runs this automatically.
...
python scripts/pipeline/generate_weekly_forecasts.py --num-weeks 30          # USD/COP /forecasting (whole-year CSV+PNG)
python scripts/pipeline/generate_asset_weekly_forecast.py --asset all --year all  # Gold/BTC /forecasting weekly_inference_<year>.json
python -m scripts.data.export_chart_ohlcv                                     # Gold/BTC /dashboard backtest-chart JSON
python scripts/pipeline/train_and_export_smart_simple.py --phase both
```

> **Regenerable `/forecasting` outputs** (untracked, per `dashboard-integration.md`): USD/COP
> `public/forecasting/bi_dashboard_unified.csv` + PNGs; per-asset
> `public/forecasting/<asset>/weekly_inference_<year>.json` (Gold/BTC, backtest-2025 + production-2026);
> `public/data/market/<SYMBOL>_daily.json` (backtest chart). A fresh clone shows `/forecasting` empty
> until the pipeline (or the `asset_<id>_pipeline_weekly` `l5_weekly_forecast` stage) regenerates them.

> **Feature-data restore (2026-07-05)**: news / analysis / `forecast_h5_*` / `asset_daily_ohlcv` /
> macro monthly+quarterly are now backed up daily to `data/backups/features/*.parquet` by
> `core_l0_05_seed_backup` and restored **empty-table-only** on cold boot (never clobbers populated
> tables). Forward pipelines (scrapers/backfill DAGs) then fill the gap to today. See
> `../specs/data/backup-recovery.md`.

---

## Schedule Coordination (Updated 2026-03-12)

> **H1 track PAUSED (audit A3-01)** — the 5 `forecast_h1_*` DAGs set `is_paused_upon_creation=True`
> and do NOT auto-run (no auto-run on DB reset). The rows below document their *intended* timing;
> production today = **H5 weekly** only.
> **Seed backup** (`core_l0_05_seed_backup`) runs `0 20 * * 1-5` (15:00 COT, **Mon-Fri only — no
> weekend backup**); the Sunday row below is annotated accordingly.
...
| — | — | `core_l0_05_seed_backup` | **Does NOT run Sunday** — seed backup is Mon-Fri only (`0 20 * * 1-5`, 15:00 COT); no weekend backup |
...
| 20:00 | 15:00 | `core_l0_05_seed_backup` | Seed backup (`0 20 * * 1-5`, Mon-Fri) |
