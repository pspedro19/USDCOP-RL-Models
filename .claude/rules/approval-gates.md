...
path** — it triggers `forecast_h5_l4b_production_deploy` (basic-auth via `AIRFLOW_API_*` env),
...
(host dev) the route falls back to the legacy local `spawn('python3', …)`; the node **container**
...
- Seeds 5 H5 DB tables via UPSERT (`--seed-db` flag) — resolves the DB URL from `DATABASE_URL` **or** builds it from `POSTGRES_*` env when unset (works from the airflow scheduler / data-seeder), registers numpy→postgres adapters, and **requires migration 054** (`UNIQUE(execution_id, subtrade_index)` for the subtrades upsert)
