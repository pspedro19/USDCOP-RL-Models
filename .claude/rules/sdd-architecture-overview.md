...
is Airflow (38 DAGs); state is PostgreSQL/TimescaleDB; artifacts are filesystem + MinIO;
...
**Load-bearing truth**: the system is *partially* multi-asset. The L0 price layer treats
`symbol` as a first-class dimension (COP/MXN/BRL already coexist in one table), but the
*tradeable asset* is hardcoded to USD/COP everywhere above L0 — features, macro drivers,
session/timezone, forecast DB tables, and the dashboard. See §7 and the onboarding spec.
...
- Compact scheduler adds `POLARS_SKIP_CPU_CHECK`, Azure/Anthropic LLM keys, extra mounts
  (`./seeds`, dashboard `public`), and Airflow pool `api_requests=8`.
...
(migration 040). See `l0-data-governance.md`.
...
mechanics, NOT model R² (audited R² < 0 both years). See `h5-smart-simple-pipeline.md`.

---

## 4. Airflow DAG Inventory (38 modules)
...
| **Forecast H1** | 6 | `forecast_h1_l3_weekly_training` (`0 6 * * 0`), `forecast_h1_l7_smart_executor` (`*/5 18-22 * * 1-5`) |
...
DAG IDs resolve from `airflow/dags/contracts/dag_registry.py`. Schedules assume the COP
...
| `/analysis` | Week selector, weekly summary, technical card, scenarios, regime, signal cards, macro charts, daily timeline, calendar, chat widget | `/api/analysis/weeks`, `/week/{y}/{w}`, `/calendar`, `/chat` | "USD/COP" subtitle; COP macro fields |
...
tradeable" is exactly the onboarding problem. → `sdd-multi-asset-onboarding.md`.
...
| Concern | Spec |
|---------|------|
| Add a new tradeable asset (BTC, Gold) | **`sdd-multi-asset-onboarding.md`** |
| Dynamic multi-strategy registry + replay (test any strategy/version/year in the UI) | **`sdd-strategy-lifecycle-registry.md`** |
| L0 data governance (OHLCV + macro) | `l0-data-governance.md` |
| H5 production pipeline | `h5-smart-simple-pipeline.md` |
| RL training pipeline | `l2-l3-l4-training-pipeline.md`, `l1-l5-inference-pipeline.md` |
| Strategy schemas | `sdd-strategy-spec.md` |
| Approval gates | `sdd-approval-spec.md` |
| Dashboard file schemas | `sdd-dashboard-integration.md` |
| Master lifecycle | `sdd-mlops-lifecycle.md` |
| Risk / execution / observability / cicd | `sdd-risk-management.md`, `sdd-execution-bridge.md`, `sdd-observability.md`, `sdd-cicd-testing.md` |
