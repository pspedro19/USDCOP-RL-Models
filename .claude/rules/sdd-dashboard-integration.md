...
docker exec usdcop-airflow-scheduler test -f /opt/airflow/scripts/generate_weekly_forecasts.py && echo "Scripts mount OK"
```
...
`production/*.json` above (which are never modified). See `sdd-strategy-lifecycle-registry.md`.
...
### Analysis Data (`public/data/analysis/`)

```
public/data/analysis/
├── analysis_index.json                  <- Available weeks list (AnalysisIndex)
├── weekly_2026_W09.json                 <- Full week export (WeeklyViewData)
├── weekly_2026_W08.json                 <- One JSON per week
├── upcoming_events.json                 <- Next week's economic calendar
└── charts/
    ├── macro_dxy_2026_W09.png           <- Macro variable PNGs with SMA/BB overlays
    ├── macro_vix_2026_W09.png
    └── ...
```

**Generated by**: `python scripts/generate_weekly_analysis.py`
...
wraps `python scripts/generate_weekly_forecasts.py --week <current>` as subprocess.
