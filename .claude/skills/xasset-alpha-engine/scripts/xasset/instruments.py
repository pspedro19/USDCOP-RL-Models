...
REGISTRY: dict[str, Instrument] = {
    ...
    # --- FX emerging ---
    # USD/COP is this repo's primary production asset (config/assets/usdcop.yaml,
    # contract CTR-ASSET-PROFILE-001). It is defined here so sizing does not fall
    # through to a KeyError on the one pair that matters most.
    #
    # pip_size is deliberately LEFT UNSET. The repo contradicts itself on what a
    # COP pip is:
    #   src/core/constants.py:403   USDCOP_PIP_VALUE = 1.0   ("1 COP pip")
    #   config/mt5_config.yaml:64   point: 0.01, pip_in_points: 1  -> pip = 0.01
    # That is a factor of 100. `size_from_risk` does not need pip_size and works
    # correctly; `pip_value()` will raise until a human resolves the conflict.
    # Guessing here would produce a position 100x wrong that still looks plausible.
    "USDCOP": Instrument(
        symbol="USDCOP",
        asset_class=AssetClass.FX_EM,
        base_ccy="USD",
        quote_ccy="COP",
        unit=Unit.BASE_CCY_UNITS,
        point_value=1.0,
        tick_size=0.01,          # agreed by both sources
        pip_size=None,           # contested - see above
        description="Colombian peso - primary production asset of this repo",
        notes=(
            "Session is exchange_hours 08:00-12:55 America/Bogota (5h, 60 M5 bars), "
            "NOT 24/5 like G10 FX - see config/assets/usdcop.yaml. Carry drivers are "
            "IBR/TPM vs Fed funds with EMBI Colombia as the country-risk premium; "
            "cross-asset leaders are USD/MXN and USD/BRL."
        ),
    ),
    "USDMXN": _fx(
