...
def _repo_metrics():
    """The constitutional SSOT for PSR/DSR — `services/common/metrics.py`.

    `.claude/rules/quant-constitution.md` names that exact path as the only source for edge
    claims, and it gates releases in production pipelines. This module therefore must NOT
    carry its own implementation: two implementations of the gate is worse than none, because
    the caller cannot tell which number they got.

    Failing loudly beats silently computing a different DSR than the one that gates releases.
    """
    import sys
    ...
    try:
        ...
        raise ImportError(
            "Cannot import services.common.metrics, the constitutional SSOT for Deflated "
            "Sharpe (.claude/rules/quant-constitution.md). Run from the repo root. This skill "
            "deliberately does NOT ship a fallback implementation."
        ) from exc
    return metrics
...
def probabilistic_sharpe_ratio(returns, sr_benchmark: float = 0.0) -> float:
    ...
    Thin ergonomic wrapper: takes raw returns and delegates the maths to
    `services.common.metrics.probabilistic_sharpe_ratio`, which additionally corrects for
    skew and excess kurtosis — this module's original version did not, and understated the
    standard error on fat-tailed return series.
    """
    m = _repo_metrics()
    r = _clean(returns)
    n = len(r)
    sr = float(r.mean() / r.std(ddof=1))
    return float(m.probabilistic_sharpe_ratio(
...
def deflated_sharpe_ratio(returns, trial_sharpes: Sequence[float]) -> dict:
    """Trial-aware Deflated Sharpe — **delegates to the constitutional SSOT**.

    Pass EVERY non-annualised trial Sharpe you computed while searching, including the ones
    you discarded. That count is the whole point: DSR deflates by how hard you looked.

    Returns the SSOT's dict ``{sr0, dsr, significant}`` — NOT a bare float. It used to return
    a float computed here, which meant a caller could get a different number from the one that
    gates releases in production. `.claude/rules/quant-constitution.md` requires
    ``services/common/metrics.py::deflated_sharpe_ratio``; this is now a wrapper over it.

    Bar: ``significant`` is True only when ``dsr > 0.95``.
    """
    m = _repo_metrics()
    ...
    if trials.size < 2:
        raise ValueError(
            "need >= 2 trial Sharpes to estimate their dispersion. If you truly "
            "ran one untuned strategy, use probabilistic_sharpe_ratio instead."
        )
    r = _clean(returns)
    return m.deflated_sharpe_ratio(
...
def validate(
    ...
    try:
        # The SSOT returns {sr0, dsr, significant} - not a bare float. `significant`
        # is the constitutional bar (dsr > 0.95) and is preferred over comparing
        # against `min_dsr` locally: re-implementing the threshold here is how the
        # two-implementations problem starts again. `min_dsr` is only consulted
        # when the caller deliberately tightens it above the constitutional bar.
        dsr_result = deflated_sharpe_ratio(r, trial_sharpes)
        dsr = float(dsr_result["dsr"])
        metrics["deflated_sharpe"] = dsr
        metrics["dsr_null_sr0"] = float(dsr_result["sr0"])
        if not dsr_result["significant"] or dsr < min_dsr:
            reasons.append(
                f"Deflated Sharpe {dsr:.3f} (bar {max(min_dsr, 0.95):.2f}, null "
                f"sr0={dsr_result['sr0']:.4f}): not distinguishable from the best "
                f"of {len(list(trial_sharpes))} trials."
            )
