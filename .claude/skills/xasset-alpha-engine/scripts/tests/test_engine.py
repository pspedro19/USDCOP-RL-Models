...
def test_usdcop_is_registered():
    """The repo's primary production asset must not fall through to KeyError."""
    i = get_instrument("USDCOP")
    assert i.is_em
    assert i.quote_ccy == "COP"
    assert i.tick_size == 0.01
    pos = i.size_from_risk(entry=4015.0, stop=3975.0, risk_budget=1_000_000.0)
    assert pos.quantity == pytest.approx(25_000.0)   # 1e6 COP risk / 40 COP stop
    assert pos.risk_at_stop == pytest.approx(1_000_000.0)


def test_usdcop_pip_value_refuses_until_repo_conflict_is_resolved():
    """src/core/constants.py says a COP pip is 1.0; mt5_config.yaml implies 0.01.

    A factor of 100. Until a human picks one, pip_value must raise rather than
    return a plausible-looking number that sizes a position 100x wrong.
    """
    with pytest.raises(ValueError, match="pip_value is FX-only"):
        get_instrument("USDCOP").pip_value(100_000)
