...
def test_dsr_rejects_best_of_many_noise_trials():
    ...
    res = deflated_sharpe_ratio(winner, trials)
    assert res["dsr"] < 0.95, f"DSR {res['dsr']:.3f} let a pure-noise winner through"
    assert res["significant"] is False


def test_dsr_returns_the_ssot_contract():
    """The return shape IS the contract - it comes straight from the SSOT.

    `services/common/metrics.py::deflated_sharpe_ratio` returns
    {sr0, dsr, significant}. This module must propagate that dict unchanged
    rather than flattening it to a float: `.claude/rules/quant-constitution.md`
    names that function as the only source for edge claims, and a caller who
    gets a bare float here cannot tell whether they got the gated number.

    This test exists because flattening it once already broke `validate()`.
    """
    res = deflated_sharpe_ratio(RNG.normal(0.0012, 0.01, 1500),
                                list(RNG.normal(0.0, 0.03, 40)))
    assert set(res) == {"sr0", "dsr", "significant"}
    assert isinstance(res["significant"], bool)
    assert res["significant"] == (res["dsr"] > 0.95)


def test_dsr_below_psr_when_many_trials():
    r = RNG.normal(0.0012, 0.01, 2000)
    trials = list(RNG.normal(0.0, 0.03, 150))
    assert deflated_sharpe_ratio(r, trials)["dsr"] < probabilistic_sharpe_ratio(r)
