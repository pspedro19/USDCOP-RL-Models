...
BUCKET_L2 = "02-l2-ds-usdcop-prepare"
BUCKET_L3B = "03-l3-ds-usdcop-feature"
L3B_SUBFOLDER = "llm-features"  # Subfolder to differentiate from RL features
...
def discover_l2_inputs(**context):
    """Discover latest L2 data from actual pipeline structure"""
    logger.info("🔍 Discovering L2 inputs from usdcop_m5__03_l2_prepare/latest/...")

    s3_hook = S3Hook(aws_conn_id='minio_conn')

    # L2 saves data in: usdcop_m5__03_l2_prepare/latest/
    l2_base_path = "usdcop_m5__03_l2_prepare/latest/"

    # Check for required files
    required_files = {
        'prepared_premium.parquet': 'STRICT dataset (no gaps)',
        'prepared_premium_flex.parquet': 'FLEX dataset (with filled gaps)',
        'metadata.json': 'Pipeline metadata'
    }

    missing_files = []
    for fname, description in required_files.items():
        full_key = f"{l2_base_path}{fname}"
        if not s3_hook.check_for_key(full_key, bucket_name=BUCKET_L2):
            logger.warning(f"⚠️ Missing {fname} ({description})")
            missing_files.append(fname)
        else:
            logger.info(f"✅ Found {fname}")

    if len(missing_files) > 0:
        raise ValueError(f"Missing L2 files: {missing_files}. Run L2 pipeline first.")

    # Read metadata to get date and run_id
    metadata_key = f"{l2_base_path}metadata.json"
    obj = s3_hook.get_key(metadata_key, bucket_name=BUCKET_L2)
    metadata = json.loads(obj.get()['Body'].read())

    l2_date = metadata.get('execution_date', context['ds'])
    l2_run_id = metadata.get('run_id', 'unknown')

    logger.info(f"✅ L2 Data Found:")
    logger.info(f"   Date: {l2_date}")
    logger.info(f"   Run ID: {l2_run_id}")
    logger.info(f"   Path: {l2_base_path}")

    context['ti'].xcom_push(key='l2_date', value=l2_date)
    context['ti'].xcom_push(key='l2_base_path', value=l2_base_path)
    context['ti'].xcom_push(key='l2_run_id', value=l2_run_id)
...
def compute_llm_features_for_split(split: str, **context):
    ...
    Now reads from L2 STRICT dataset and performs the split internally.
    """
    logger.info(f"📊 Computing LLM features for {split}...")

    s3_hook = S3Hook(aws_conn_id='minio_conn')

    # Load L2 STRICT data (premium quality, no gaps)
    l2_base_path = context['ti'].xcom_pull(key='l2_base_path')
    l2_key = f"{l2_base_path}prepared_premium.parquet"

    logger.info(f"Reading L2 data from: {l2_key}")
    obj = s3_hook.get_key(l2_key, bucket_name=BUCKET_L2)
    data = obj.get()["Body"].read()
    df_full = pd.read_parquet(io.BytesIO(data))

    logger.info(f"Loaded {len(df_full)} rows, {df_full['episode_id'].nunique()} episodes")

    # Split data by episode_id (70% train, 15% val, 15% test)
    unique_episodes = sorted(df_full['episode_id'].unique())
    n_episodes = len(unique_episodes)

    train_cutoff = int(n_episodes * 0.70)
    val_cutoff = int(n_episodes * 0.85)

    if split == 'train':
        selected_episodes = unique_episodes[:train_cutoff]
    elif split == 'val':
        selected_episodes = unique_episodes[train_cutoff:val_cutoff]
    else:  # test
        selected_episodes = unique_episodes[val_cutoff:]

    df = df_full[df_full['episode_id'].isin(selected_episodes)].copy()
    logger.info(f"Split {split}: {len(df)} rows, {len(selected_episodes)} episodes")

    logger.info(f"Loaded {len(df)} rows from L2 {split}")

    # Ensure required columns exist
    required = ['episode_id', 't_in_episode', 'open', 'high', 'low', 'close']
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns in L2 data: {missing}")

    # Add volume column if not present (L2 STRICT doesn't include volume)
    if 'volume' not in df.columns:
        logger.warning("⚠️ Volume column not in L2 data, setting to 0")
        df['volume'] = 0
    ...
    # Get l2_date and extract date value (handles both "2025-10-29" and "date=2025-10-29")
    l2_date = context['ti'].xcom_pull(key='l2_date')
    if '=' in str(l2_date):
        date_value = l2_date.split('=')[1]
    else:
        date_value = l2_date

    output_key = f"{L3B_SUBFOLDER}/date={date_value}/run_id={run_id}/{split}_llm_features.parquet"
...
def save_l3b_manifest(**context):
    ...
    # Extract date value (handles both "2025-10-29" and "date=2025-10-29")
    if '=' in str(l2_date):
        date_value = l2_date.split('=')[1]
    else:
        date_value = l2_date
    ...
    manifest = {
        ...
        "notes": [
            ...
            "Volume set to 0 (TwelveData doesn't provide volume for USD/COP)"
        ]
    }

    manifest_key = f"{L3B_SUBFOLDER}/date={date_value}/run_id={run_id}/manifest.json"
    ...
    s3_hook.load_string(
        json.dumps(manifest, indent=2),
        key=f"{L3B_SUBFOLDER}/_meta/latest.json",
        bucket_name=BUCKET_L3B,
        replace=True
    )
