...
# DWH Helper
sys_mod.path.insert(0, os.path.dirname(__file__))
from utils.dwh_helper import DWHHelper, get_dwh_connection

# ✅ MEJORA: MLflow Registry Helper
from utils.mlflow_registry_helper import register_rl_model
        ...
        if files_metadata:
            manifest = write_manifest(
                ...
                metadata={
                    ...
                    'pipeline': 'usdcop_m5__05a_l5_rl_training',
                    'airflow_dag_id': 'usdcop_m5__05a_l5_rl_training',
                    'dag_run_id': context['dag_run'].run_id,
                    'gates_passed': gates_passed,
...
def load_to_dwh(**context):
    """
    Load L5 serving data to Data Warehouse.

    Inserts data into:
    - dw.fact_inference_latency (latency metrics)

    Note: fact_signal_5m is typically populated by the inference service
    in real-time, not by this batch DAG.
    """
    logging.info("="*60)
    logging.info("🏛️ LOADING L5 SERVING DATA TO DWH (KIMBALL)")
    logging.info("="*60)

    try:
        # Get gate results which contain latency metrics
        gate_results = context['ti'].xcom_pull(task_ids='evaluate_production_gates', key='gate_results')

        if not gate_results:
            logging.warning("⚠️ No gate results found, skipping DWH load")
            return {'status': 'skipped', 'reason': 'no_data'}

        # Get model info from bundle
        bundle_info = context['ti'].xcom_pull(task_ids='create_model_bundle', key='bundle_info')

        # Connect to DWH
        conn = get_dwh_connection()
        dwh = DWHHelper(conn)

        # Get or create model dimension
        model_id = bundle_info.get('model_id', 'rl_ppo_v1.0') if bundle_info else 'rl_ppo_v1.0'
        model_hash = bundle_info.get('model_hash', '') if bundle_info else ''

        model_sk = dwh.get_or_insert_dim_model_scd2(
            model_id=model_id,
            model_name='Production PPO Model',
            algorithm='PPO',
            version='v1.0',
            framework='stable-baselines3',
            sha256_hash=model_hash,
            is_production=gate_results.get('passed', False) if gate_results else False
        )

        logging.info(f"✅ Model dimension: model_sk={model_sk}")

        # Extract latency metrics from gate results
        latency_metrics = {
            'latency_p50_ms': gate_results.get('latency_p50_ms'),
            'latency_p95_ms': gate_results.get('latency_p95_ms'),
            'latency_p99_ms': gate_results.get('latency_p99_ms'),
            'e2e_latency_p99_ms': gate_results.get('e2e_p99_ms'),
            'throughput_eps': gate_results.get('throughput_eps'),
            'inference_count': gate_results.get('inference_count', 0)
        }

        # Insert inference latency
        dwh.insert_fact_inference_latency(
            model_sk=model_sk,
            date_cot=context['execution_date'].date(),
            metrics=latency_metrics,
            dag_id='usdcop_m5__05a_l5_rl_training'
        )

        logging.info(f"✅ Inserted fact_inference_latency")
        ...
        result = {
            ...
            'model_sk': model_sk,
            'model_id': model_id,
            'latency_inserted': True,
            'is_production': gate_results.get('passed', False) if gate_results else False
        }

        logging.info("="*60)
        logging.info("🎉 L5 DWH LOAD COMPLETED SUCCESSFULLY")
        logging.info(f"📊 Model: {model_id}")
        logging.info(f"📊 Production ready: {result['is_production']}")
        logging.info("="*60)

        return result

    except Exception as e:
        logging.error(f"❌ L5 DWH load failed: {e}")
        import traceback
...
dag = DAG(
    'usdcop_m5__05a_l5_rl_training',
    default_args=default_args,
    description='L5A: RL training (PPO-LSTM) with Sortino metrics and L4-based costs',
    schedule_interval=None,
    catchup=False,
    max_active_runs=1,
    tags=['l5a', 'rl', 'ppo-lstm', 'training', 'production', 'gates', 'sortino'],
)
...
# Set dependencies
validate_l4 >> prepare_env >> ppo_group >> gate_eval >> bundle_task >> finalize >> load_dwh