# -*- coding: utf-8 -*-
"""
Fedesarrollo Extractor - Colombian Economic Research Center.

Extracts:
- CCI (Consumer Confidence Index) - crsk_sentiment_cci_col_m_cci
- ICI (Industrial Confidence Index) - crsk_sentiment_ici_col_m_ici
- EOF Inflation Expectations - infl_exp_eof_col_m_infexp

Uses the existing scraper_fedesarrollo.py module which downloads
PDF reports from Fedesarrollo's repository.
"""
...
class FedesarrolloExtractor(BaseExtractor):
    """Fedesarrollo Confidence Index & Inflation Expectations Extractor."""
    ...
    def _load_scraper(self):
        """Lazy load the Fedesarrollo scraper module."""
        if self._scraper_module is not None:
            return self._scraper_module

        try:
            import importlib.util
            scraper_path = SCRAPERS_PATH / 'scraper_fedesarrollo.py'

            if not scraper_path.exists():
                logger.error("[FEDESARROLLO] Scraper module not found: %s", scraper_path)
                return None

            spec = importlib.util.spec_from_file_location("scraper_fedesarrollo", scraper_path)
            self._scraper_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(self._scraper_module)

            logger.info("[FEDESARROLLO] Scraper module loaded successfully")
            return self._scraper_module

        except Exception as e:
            logger.error("[FEDESARROLLO] Failed to load scraper module: %s", e)
            return None
    ...
    def extract(
        ...
        """Extract data from Fedesarrollo via scraper."""
        ...
        if scraper is None:
            return ExtractionResult(
                ...
                data=pd.DataFrame(),
                success=False,
                error="Could not load Fedesarrollo scraper module"
            )
        ...
        try:
            ...
            request_n = last_n * 2 if last_n else 100
            df = func(n=request_n)
            ...
            # For monthly data, don't filter by date range as it has
            # ~30-45 days publication delay. Just take the last N records.
            ...
            logger.error("[FEDESARROLLO] Failed to extract %s: %s", variable, e)
            return ExtractionResult(
                source=self.source_name,
                variable=variable,
                data=pd.DataFrame(),
    ...
    def get_latest_date(self, variable: str) -> Optional[datetime]:
        ...
        try:
            ...
            logger.warning("[FEDESARROLLO] Failed to get latest date for %s: %s", variable, e)
