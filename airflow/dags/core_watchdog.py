...
def check_ohlcv_freshness(**context):
    ...
    try:
        ...
        if row and row[1] is not None:
            ...
            if minutes_ago > 15:
                logger.warning(
                    f"[Watchdog] OHLCV STALE: {minutes_ago:.0f} min ago. Triggering realtime DAG."
                )
                return {
                    "status": "stale",
                    "minutes_ago": minutes_ago,
                    "action": "trigger_ohlcv_realtime",
                }
...
def check_macro_freshness(**context):
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT MAX(fecha), (CURRENT_DATE - MAX(fecha)) as days_ago FROM macro_indicators_daily"
        )
        row = cur.fetchone()
        if row and row[1] is not None:
...
def check_forecasting_freshness(**context):
    ...
    if latest_week < current_week:
        logger.warning(
            f"[Watchdog] Forecasting STALE: latest={latest_week}, current={current_week}"
        )
        return {
            "status": "stale",
            "latest": latest_week,
...
def check_h5_signal(**context):
    ...
    try:
        ...
        if row:
            logger.info(
                f"[Watchdog] H5 signal OK: {row[0]}, dir={row[1]}, skip={row[2]}, regime={row[4]}"
            )
    ...
    for name, result in checks.items():
        ...
        try:
            if action == "trigger_ohlcv_realtime":
                ...
                actions_taken.append("Triggered OHLCV realtime")
