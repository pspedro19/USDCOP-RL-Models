...
ALL_FEATURES = TIER_1_FEATURES + TIER_2_FEATURES  # 8 + 8 = 16 features total (orthogonal)
...
def load_strict_dataset(**context):
    ...
    temp_key = f"temp/l3_feature/{run_id}/input_data.parquet"
    buffer = io.BytesIO()
...
def calculate_tier1_features(**context):
    ...
    tier1_key = f"temp/l3_feature/{run_id}/tier1_features.parquet"
    buffer = io.BytesIO()
...
def calculate_tier2_features(**context):
    ...
    all_features_key = f"temp/l3_feature/{run_id}/all_features.parquet"
    buffer = io.BytesIO()
    df_features.to_parquet(buffer, index=False)
    buffer.seek(0)
...
def run_comprehensive_causality_tests_task(**context):
    ...
    causality_test_key = f"usdcop_m5__04_l3_feature/_reports/causality_tests.json"
    s3_hook.load_string(
    ...
    leakage_gate_key = f"usdcop_m5__04_l3_feature/_reports/leakage_gate.json"
    s3_hook.load_string(
...
def validate_forward_ic(**context):
    ...
    forward_ic_key = f"usdcop_m5__04_l3_feature/_reports/forward_ic_report.json"
    s3_hook.load_string(
    ...
    target_alignment_key = f"usdcop_m5__04_l3_feature/_reports/target_alignment_verification.json"
    ...
    if not ic_detail_df.empty:
        ...
        ic_csv_key = f"usdcop_m5__04_l3_feature/_reports/per_feature_ic_tracking.csv"
        s3_hook.load_string(
    ...
    sentinel_key = f"usdcop_m5__04_l3_feature/_reports/sentinel_checks.json"
    s3_hook.load_string(
    ...
    lineage_key = f"usdcop_m5__04_l3_feature/_reports/feature_lineage_metadata.json"
    s3_hook.load_string(
...
def apply_quality_gates(**context):
    ...
    # Save quality report
    quality_key = f"usdcop_m5__04_l3_feature/_reports/quality_report.json"
    s3_hook.load_string(
    ...
    corr_key = f"usdcop_m5__04_l3_feature/_reports/correlation_matrix.json"
...
def save_final_features(**context):
    ...
    final_key = f"usdcop_m5__04_l3_feature/market=usdcop/timeframe=m5/date={execution_date}/run_id={run_id}/features.parquet"
    ...
    # Save all reports
    reports_path = f"usdcop_m5__04_l3_feature/_reports/"
    
    # Save feature specification
    spec_key = f"usdcop_m5__04_l3_feature/_metadata/feature_spec.json"
    s3_hook.load_string(
    ...
    # Save metadata.json
    metadata_key = f"usdcop_m5__04_l3_feature/_metadata/metadata.json"
    s3_hook.load_string(
    ...
    schema_key = f"usdcop_m5__04_l3_feature/_metadata/train_schema.json"
    s3_hook.load_string(
    ...
    if normalization_ref:
        norm_ref_l3_key = f"usdcop_m5__04_l3_feature/_metadata/normalization_ref.json"
        s3_hook.load_string(
    ...
    audit_summary = {
        ...
        "files_created": {
            ...
            "audit_summary": f"s3://{BUCKET_OUTPUT}/usdcop_m5__04_l3_feature/_reports/audit_summary.json"
        }
    }
    
    # Save comprehensive audit summary
    audit_summary_key = f"usdcop_m5__04_l3_feature/_reports/audit_summary.json"
    s3_hook.load_string(
    ...
    staging_parquet_key = f"usdcop_m5__04_l3_feature/latest/_staging/features.parquet"
    staging_csv_key = f"usdcop_m5__04_l3_feature/latest/_staging/features.csv"
    ...
    latest_parquet_key = f"usdcop_m5__04_l3_feature/latest/features.parquet"
    latest_csv_key = f"usdcop_m5__04_l3_feature/latest/features.csv"
    ...
    for metadata_file in ['feature_spec.json', 'metadata.json', 'train_schema.json']:
        src_key = f"usdcop_m5__04_l3_feature/_metadata/{metadata_file}"
        if s3_hook.check_for_key(src_key, bucket_name=BUCKET_OUTPUT):
            obj = s3_hook.get_key(src_key, bucket_name=BUCKET_OUTPUT)
            content = obj.get()['Body'].read()
            latest_key = f"usdcop_m5__04_l3_feature/latest/_metadata/{metadata_file}"
            s3_hook.load_bytes(
                bytes_data=content,
                key=latest_key,
                bucket_name=BUCKET_OUTPUT,
    ...
    for report_file in report_files:
        src_key = f"usdcop_m5__04_l3_feature/_reports/{report_file}"
        if s3_hook.check_for_key(src_key, bucket_name=BUCKET_OUTPUT):
            obj = s3_hook.get_key(src_key, bucket_name=BUCKET_OUTPUT)
            content = obj.get()['Body'].read()
            latest_key = f"usdcop_m5__04_l3_feature/latest/_reports/{report_file}"
            s3_hook.load_bytes(
                bytes_data=content,
                key=latest_key,
                bucket_name=BUCKET_OUTPUT,
...
# AUDITOR REQUIREMENT: Updated task dependencies with causality tests before forward IC
task_load_data >> task_tier1 >> task_tier2 >> task_causality_tests >> task_forward_ic >> task_quality >> task_save