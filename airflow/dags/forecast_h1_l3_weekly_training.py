...
MLFLOW_EXPERIMENT_NAME = "h1_weekly_training"
...
def _get_git_commit_hash() -> Optional[str]:
    ...
    try:
        ...
        logger.debug(f"[H1-L3] Could not resolve git commit: {e}")
    return None
...
def _mlflow_safe_start_run(run_name: str, experiment_name: str = MLFLOW_EXPERIMENT_NAME):
    ...
    try:
        ...
        logger.info(
            f"[H1-L3] MLflow run started: experiment={experiment_name}, "
            f"run_name={run_name}, run_id={active_run.info.run_id}"
        )
        return mlflow, active_run
    except Exception as e:
        logger.warning(
            f"[H1-L3] MLflow tracking unavailable (non-blocking): {type(e).__name__}: {e}"
        )
        return None, None
...
def _mlflow_safe_end_run(mlflow_mod, status: str = "FINISHED") -> None:
    ...
    try:
        ...
        logger.warning(f"[H1-L3] MLflow end_run failed (non-blocking): {e}")
...
def _mlflow_safe_call(mlflow_mod, method: str, *args, **kwargs) -> None:
    ...
    try:
        ...
        logger.warning(f"[H1-L3] MLflow {method} failed (non-blocking): {e}")
...
def train_models(**context) -> Dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    train_start_date = df["date"].iloc[0].date() if len(df) else None
    train_end_date = df["date"].iloc[-1].date() if len(df) else None
    ...
    run_name = f"h1_weekly_W{iso_week:02d}_{iso_year}_{run_timestamp}"
    ...
    _mlflow_safe_call(mlflow_mod, "log_params", {
        ...
        "seed": 42,
        "horizon_n_estimators": horizon_config.get("n_estimators"),
        "horizon_max_depth": horizon_config.get("max_depth"),
        "horizon_learning_rate": horizon_config.get("learning_rate"),
    })
    _mlflow_safe_call(mlflow_mod, "set_tag", "pipeline", "h1_weekly")
    _mlflow_safe_call(mlflow_mod, "set_tag", "dag_id", DAG_ID)
    _mlflow_safe_call(mlflow_mod, "set_tag", "contract", "FC-H1-L3-001")
    ...
    try:
        ...
        for model_id in MODEL_IDS:
            try:
                ...
                logger.info(
                    f"[L5a] Trained {model_id}: in-sample DA={in_sample_da:.1f}%, "
                    f"R2={in_sample_r2:.4f}"
                )
        ...
        _mlflow_safe_call(mlflow_mod, "log_metric", "models_total", float(len(MODEL_IDS)))
