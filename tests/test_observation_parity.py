...
try:
    ...
    try:
        from core.builders.observation_builder import ObservationBuilder
    except ImportError:
        print("CRITICAL: Unable to import ObservationBuilder. Check paths.")
        sys.exit(1)

def test_parity():
    print("="*60)
    print("OBSERVATION PARITY TEST")
    print("="*60)

    try:
        print("Initializing ObservationBuilder...")
        builder = ObservationBuilder(config_path=os.path.join(project_root, 'config', 'feature_config.json'))
        ...
        if obs.shape == expected_shape:
            print(f"\n PASS: Observation dimension matches spec {expected_shape}.")
        else:
