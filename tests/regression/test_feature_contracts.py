...
3. The §41-47 matrix is a CI fixture: v11 recipe = 25 features (exact ordered list),
   the H5-L3 DAG snapshot = 23 (exact ordered list), and the divergence is DECLARED
   and LOCKED — not resolved (BL-14 drift_note in config/strategy_manifests/usdcop.yaml).
   Rule-based champions declare their minimal input set (MA200 solo close).
...
def test_catalog_code_references_exist_and_hashes_match():
    ...
    for f in cat["features"]:
        ...
        current = seen.setdefault(path, _sha16(ROOT / path))
...
# ═══════════════════════════════════════════════════════════════════════════
# 2. §41-47 matrix as CI fixture — v11 = 25, snapshot = 23, drift DECLARED
# ═══════════════════════════════════════════════════════════════════════════

def test_v11_feature_set_matches_frozen_recipe():
    fs = _feature_set("usdcop_smart_simple_v11")
...
def test_v11_dag_snapshot_is_declared_and_locked_not_resolved():
    """BL-14 declared a REAL divergence (recipe 25 vs DAG snapshot 23). BL-39 must
    DECLARE and LOCK it — resolving it silently here is prohibited (0 trials)."""
    fs = _feature_set("usdcop_smart_simple_v11")
    snap = fs["dag_snapshot"]
    assert snap["ordered_features"] == SNAPSHOT_23, (
        "pinned DAG snapshot must be the exact 23-column list of feature_cols_h5.json")
    assert snap["file_sha256_16"] == FEATURE_COLS_JSON_SHA16
    ...
    # Cross-lock with the frozen manifest (BL-14): numbers and hash must agree.
...
def test_disk_artifacts_are_bit_identical_to_frozen_registration():
    ...
    assert _sha16(ARTIFACTS_DIR / "feature_cols_h5.json") == FEATURE_COLS_JSON_SHA16
    assert _sha16(ARTIFACTS_DIR / "scaler_h5.pkl") == SCALER_SHA16
    manifest = yaml.safe_load(MANIFEST_PATH.read_text(encoding="utf-8"))
    expected = manifest["components"][0]["current_model_snapshot"]["artifacts_sha256_16"]
    for fname in ("ridge_h5.pkl", "bayesian_ridge_h5.pkl"):
        assert _sha16(ARTIFACTS_DIR / fname) == expected[fname], (
