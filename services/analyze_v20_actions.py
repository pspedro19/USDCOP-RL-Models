"""
Analyze V20 Model Action Distribution
=====================================
Diagnose why V20 generates so few trades by analyzing action values.
"""
...
# Import from paper_trading_v20
from paper_trading_v20 import (
    load_ohlcv_data, load_macro_data, FeatureCalculator,
    MARKET_FEATURES, CONFIG, MODEL_PATH
)

def analyze_actions():
    """Analyze what action values the V20 model outputs."""
    from stable_baselines3 import PPO

    print("=" * 70)
    print("ANALYZING V20 MODEL ACTION DISTRIBUTION")
    print("=" * 70)
    ...
    for i in range(warmup, len(features_df)):
        ...
        # Build observation (15-dim for V20)
    ...
    print(f"Para ~4 trades/día (32 total), usar threshold: ±{best_thresh:.2f}")

    # Also show what happens with different thresholds
    print("\nSimulación de trades con diferentes umbrales:")
    for thresh in [0.10, 0.15, 0.20]:
        ...
        for a in actions:
            ...
            if new_pos != prev_pos and new_pos != "FLAT":
                trades += 1
            prev_pos = new_pos
        print(f"  Threshold ±{thresh:.2f}: ~{trades} trades en el período")
