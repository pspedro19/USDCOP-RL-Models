"""
Paper Trading Simulation V20 Enhanced
=====================================

Enhanced version that handles V20's directional bias by:
...
MODEL_PATH = PROJECT_ROOT / "models" / "ppo_v20_production" / "final_model.zip"

# Trading parameters - Optimized for V20's behavior
...
class PaperTradingEngineV20Enhanced:
    """Enhanced paper trading that handles V20's directional bias."""
    ...
    def load_model(self):
        ...
        logger.info(f"Loading V20 model from {MODEL_PATH}")
    ...
    def run_simulation(self, ohlcv_df: pd.DataFrame, macro_df: pd.DataFrame) -> Dict:
        logger.info(f"Starting ENHANCED V20 simulation with {len(ohlcv_df)} bars")
        ...
        logger.info("=" * 60)
        logger.info("V20 ENHANCED SIMULATION COMPLETE")
...
def save_results_to_db(results: Dict):
    ...
    try:
        ...
        conn.commit()
        logger.info("V20 ENHANCED results saved to database")
...
def main():
    print("=" * 70)
    print("V20 ENHANCED PAPER TRADING SIMULATION")
    print("=" * 70)
    ...
    engine = PaperTradingEngineV20Enhanced(CONFIG)
    results = engine.run_simulation(ohlcv_df, macro_df)
    save_results_to_db(results)

    print("\n" + "=" * 70)
    print("V20 ENHANCED RESULTS")
    print("=" * 70)
