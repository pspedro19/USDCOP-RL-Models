...
**`/forecasting` is multi-asset** (pair selector), branched by `analysis-assets.ts::forecast_mode` — **the branch is `forecast_mode`, never a hardcoded asset id**: **USD/COP + Gold + BTC = 9-model ML zoo** (CSV+PNG, whole-year via `generate_weekly_forecasts.py --asset <id> --num-weeks 30` → root for COP, `public/forecasting/{xauusd,btcusdt}/` for the others; `AssetModelZoo`). Gold and BTC **also** publish `weekly_inference_<year>.json` (`generate_asset_weekly_forecast.py`, `AssetWeeklyBody`) — a **secondary** surface, not the branch's input. `spx500` is `forecast_mode: 'none'`: it is analysed but publishes **no** forecasting artifacts, so `FORECASTING_ASSETS` keeps it out of the selector while `ANALYSIS_ASSETS` (SSOT shared with `/analysis`) still carries it. *(Corrected 2026-08-05: this line used to claim "Gold = rule-based weekly inference" while `analysis-assets.ts` sent Gold to the zoo and disk held 459 zoo artifacts for it — measure what is served, not what is declared.)* **BTC uses a BTC-appropriate 19-feature set** (17 price/technical/calendar + DXY + VIX; drops the Colombia-only WTI/EMBI; √365; config `config/assets/btcusdt_forecasting.yaml`) — same STRUCTURE as COP, honest features. BTC price-only DA ≈ 0.46 (a transparency surface, **not** an edge claim — quant-constitution). **Methodology (all pairs): trained ≤ Dec-2024, 2025 = backtest (OOS, default), 2026 = production.** See `dashboard-integration.md`.
