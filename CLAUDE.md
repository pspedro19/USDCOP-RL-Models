...
1. **H5 Weekly Pipeline (PRODUCTION, v11 FROZEN)**: Smart Simple v2.0, Ridge+BR + Regime Gate (XGBoost = experimento offline, `use_xgboost: false` — `smart_simple_v1.yaml:206`)
   ...
   - **2026 forward (único juez limpio)**: **+0.66% YTD, 12 trades hasta 2026-W33** (último 2026-08-10, $10K → $10,066). Es lo que reproduce hoy `scripts/pipeline/candidates_paper_ledger.py` con el seed al 2026-09-10. **El artefacto publicado está desfasado**: `production/paper/candidates_ledger_2026.json` (2026-08-11) dice +2.90%/11 trades y no se puede regenerar sin la base de datos, porque el linaje BL-24 se persiste atómicamente con el fichero. Replay 2026 = +3.36%, 11 trades (`summary.json`). **N<20 ⇒ solo conteo y PnL** (constitución §6). Corte A del retiro (26 sem) ≈ **2026-09-16**.
...
- H5 Weekly v2.0 (`smart_simple_v11`): $10K → $10,735 (2025 OOS oficial), $10K → $10,066 (2026 forward paper, W33, recomputado)
- Candidatas v12/v14: replay descriptivo + forward post-freeze, +0.43% / +0.40% YTD recomputado — ninguna bate a v11
