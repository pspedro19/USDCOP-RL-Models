"""
Portafolio Scraper (SDD-02 §2.7)
==================================
Colombian business newspaper (Grupo Tiempo). RSS + sitemap scraping.
Focuses on: Economia, Finanzas, Negocios sections.
"""
...
class PortafolioScraper(SourceAdapter):
    """Portafolio RSS + sitemap scraper."""
    ...
    def fetch_latest(self, hours_back: int = 24) -> list[RawArticle]:
        ...
        if not articles:
            try:
                ...
                logger.warning(f"[portafolio] Sitemap failed: {e}")
    ...
    def _parse_rss(self, cutoff: datetime) -> list[RawArticle]:
        """Parse Portafolio RSS feed."""
        try:
            import feedparser
        except ImportError:
            logger.error("[portafolio] feedparser not installed")
            return []
    ...
    def _parse_sitemap(self, cutoff: datetime) -> list[RawArticle]:
        ...
        try:
            ...
            logger.error("[portafolio] beautifulsoup4 not installed")
            return []

        resp = requests.get(
            self.cfg.portafolio_sitemap,
            headers={"User-Agent": self.cfg.user_agent},
            timeout=self.cfg.request_timeout,
        )
    ...
    def scrape_article_content(self, url: str) -> str | None:
        ...
        try:
            ...
            logger.warning(f"[portafolio] Scrape failed for {url}: {e}")
            return None
