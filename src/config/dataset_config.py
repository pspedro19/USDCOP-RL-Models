"""
Dataset Configuration - USDCOP Trading
======================================

Dataset specification for model training.
...
DATASET_CONFIG = {
    ...
    # Feature list (15 dimensions: 13 core + 2 state)
    "features": {
        "core_market": [
    ...
    "normalization": {
        ...
        "stats_file": "config/norm_stats.json",
    },
    ...
    "filters": {
        ...
        "exclude_us_holidays": True,     # Include US holidays (affects USD)
...
def get_date_range(split: str):
    ...
    """
    ranges = DATASET_CONFIG["date_ranges"]
...
if __name__ == "__main__":
    ...
    print(f"  Total features: {DATASET_CONFIG['features']['total_dim']}")
    print(f"  Feature list: {get_feature_list()}")
    print("\nDate Ranges:")
    for split, config in DATASET_CONFIG["date_ranges"].items():
        print(f"  {split}: {config['start']} to {config['end']}")
    print(f"\nFilters: Market hours {DATASET_CONFIG['filters']['market_open']} - {DATASET_CONFIG['filters']['market_close']} COT")
