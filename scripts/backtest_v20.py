#!/usr/bin/env python3
"""
V20 Backtest Validation Script
==============================

Runs out-of-sample backtest on the trained V20 model.
...
def normalize_observation(
    ...
    """
    Normalize observation to match production ObservationBuilderV19.
    """
    obs = np.zeros(V19_OBS_DIM, dtype=np.float32)

    # Normalize core features
    for i, (fname, value) in enumerate(zip(V19_CORE_FEATURES, features)):
        if fname in norm_stats:
            mean = norm_stats[fname].get('mean', 0.0)
            std = norm_stats[fname].get('std', 1.0)
            if std < 1e-8:
                std = 1.0
            obs[i] = np.clip((value - mean) / std, -5.0, 5.0)
        else:
            obs[i] = np.clip(value, -5.0, 5.0)
...
def run_backtest(
    ...
    # Extract features
    features = test_df[V19_CORE_FEATURES].values.astype(np.float32)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='V20 Backtest Validation')
    parser.add_argument('--model', type=str,
                       default='models/ppo_v20_production/final_model.zip',
    ...
    parser.add_argument('--norm-stats', type=str,
                       default='config/v19_norm_stats.json',
