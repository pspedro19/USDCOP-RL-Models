...
def test_hold_reward_zero():
    ...
    try:
        ...
        if all_zero:
            print("[PASS] PASS: All HOLD rewards are 0.0 in eval mode")
        else:
            print(f"[FAIL] FAIL: HOLD rewards not zero. Average: {avg_reward:.6f}")
...
def test_loop_detector():
    ...
    try:
        ...
        if is_normal and not is_truncated:
            print(f"[PASS] PASS: Episode of {ep_length} steps with done=True marked as NORMAL")
        else:
            print(f"[FAIL] FAIL: Episode of {ep_length} steps with done=True incorrectly marked")
        ...
        if is_truncated:
            print(f"[PASS] PASS: Episode of {ep_length} steps with done=False marked as TRUNCATED")
        else:
            print(f"[FAIL] FAIL: Episode of {ep_length} steps with done=False not marked as truncated")
...
def test_sortino_robust():
    ...
    try:
        ...
        if -100 <= sortino_extreme <= 100:
            print("[PASS] PASS: Extreme Sortino values properly clamped")
        else:
            print(f"[FAIL] FAIL: Extreme Sortino not clamped: {sortino_extreme}")
...
def test_configuration():
    ...
    try:
        ...
        if 'default_var="300000"' in dag_content:
            print("[PASS] PASS: TOTAL_TIMESTEPS increased to 300000")
        else:
            print("[FAIL] FAIL: TOTAL_TIMESTEPS not properly increased")
        
        # Check cost curriculum
        if 'initial_cost_factor=0.25' in dag_content:
            print("[PASS] PASS: Cost curriculum starts at 25%")
        else:
            print("[FAIL] FAIL: Cost curriculum not properly configured")
        
        if 'TOTAL_TIMESTEPS * 0.8' in dag_content:
            print("[PASS] PASS: Cost curriculum reaches 100% at 80% of training")
        else:
            print("[FAIL] FAIL: Cost curriculum progression not properly configured")
        
        # Check hyperparameters
        if '"ent_coef": 0.005' in dag_content:
            print("[PASS] PASS: Entropy coefficient reduced to 0.005")
        else:
            print("[FAIL] FAIL: Entropy coefficient not properly set")
...
def test_sanity_check():
    ...
    try:
        ...
        # Create environment factory that accepts optional mode parameter
        def env_factory(mode="train", **kwargs):
            return USDCOPTradingEnv(mode=mode)
        
        # Run sanity check with no costs
        passes = quick_sanity_check(
        ...
        if passes:
            print("[PASS] PASS: Sanity check passed")
            print("   Mean reward is acceptable and has variance")
        else:
            print("[FAIL] FAIL: Sanity check failed")
            print("   Mean reward too negative or no variance")
...
def test_bundle_l4_artifacts():
    ...
    try:
        ...
        if 'def _ensure_l4_local' in dag_content:
            print("[PASS] PASS: _ensure_l4_local function exists")
        else:
            print("[FAIL] FAIL: _ensure_l4_local function not found")
            return False
        
        if 'split_spec = _ensure_l4_local' in dag_content:
            print("[PASS] PASS: split_spec downloaded from L4")
        else:
            print("[FAIL] FAIL: split_spec not using L4 download")
        
        if 'norm_ref = _ensure_l4_local' in dag_content:
            print("[PASS] PASS: normalization_ref downloaded from L4")
        else:
            print("[FAIL] FAIL: normalization_ref not using L4 download")
...
def main():
    ...
    if passed == total:
        print("\n[SUCCESS] SUCCESS: All tests passed! L5 pipeline is ready for production.")
    else:
        print(f"\n[WARNING]  WARNING: {total - passed} tests failed. Review and fix before deployment.")
    
    # Save results (convert bool_ to bool for JSON serialization)
