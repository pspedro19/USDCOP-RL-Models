#!/usr/bin/env python3
"""Publica `gold_trend_simple` — la misma senal SMA sin el trailing exit.

Contract: CTR-QUANT-EVIDENCE-001 · Hipotesis H-SIMP-GOLD-01 (registrada 2026-07-21 ANTES de correr)

Por que existe: la medicion de evidencia mostro que quitarle el Chandelier trailing a
`gold_dynamic_exit` mejora TODO lo medible.

  gold_dynamic_exit   DSR 0.052   muere a costos x2   a exposicion emparejada PIERDE (-0.66%)
  gold_trend_simple   DSR 0.861   sobrevive x2        a exposicion emparejada +10.94%

Y en el unico ano held-out (2025), a exposicion emparejada: 113.58% vs 62.12% de comprar y
aguantar, con Calmar 12.78 vs 5.95. Es la UNICA estrategia del libro que pasa el test de
exposicion emparejada en ambas ventanas (historia completa Y OOS).

La maquinaria de salida no era inutil: era el termino negativo dominante.

ADVERTENCIA METODOLOGICA que viaja con el bundle: la observacion que motivo esta variante se
hizo sobre 2004-2026, asi que ese periodo NO puede juzgarla (quant-constitution 1). El backtest
publicado es contexto; el juez es el forward. Por eso se publica `experimental`, nunca
`production`, con independencia de lo buenos que parezcan los numeros.

Usage: python scripts/pipeline/publish_gold_trend_simple.py [--version 1.0.0]
"""
...
from scripts.analysis.gold_trend_simple import simulate  # noqa: E402
...
NAME = "Gold · Trend simple (voto SMA, sin trailing exit)"
TRAIL_M = None         # no hay trailing exit: ES la hipotesis (H-SIMP-GOLD-01)
TRIALS_PROGRAM = 75    # 74 del programa + H-SIMP-GOLD-01 (registry front-matter)
INITIAL = 10000.0
...
def main() -> int:
    ...
    summary = {
        ...
        "methodology": ("Entrada: votos SMA{63,126,252} >= 2/3 (causal t-1). Salida: SOLO cuando "
                        "la senal muere. SIN trailing stop -- esa ausencia ES la hipotesis "
                        "H-SIMP-GOLD-01. Vol-target 10%, cap 1.5. Diseno <=2024, OOS=2025. "
                        "El backtest es CONTEXTO: la observacion que motivo esta variante se hizo "
                        "sobre el mismo historial, asi que el juez es el forward (constitucion 1)."),
    }
    ...
    # Campeona POR ACTIVO, explícita. La versión anterior usaba un set plano
    # {"smart_simple_v11", "btc_trend_b2", SID} que no mencionaba spx500 — así que publicar Oro
    # archivó la única estrategia de SPX500 y dejó ese activo SIN NINGUNA herramienta visible,
    # en silencio. Un set plano no puede expresar "uno por activo": no sabe qué activos existen.
    #
    # Elección por evidencia OOS-2025 (constitución §3: "si la estrategia no lo bate, el
    # baseline ES la estrategia"):
    #   usdcop  smart_simple_v11        congelada, forward corriendo desde 2026-03-18
    #   xauusd  gold_trend_simple       única que pasa exposición-emparejada en AMBAS ventanas
    #   btcusdt btc_hodl_b1             +4.70% OOS vs -1.37%..-2.83% de todas las variantes
    #   spx500  spx500_regime_gated_v1  no bate a MA200 pelado, pero es lo único medido sobre
    #                                   datos reales; se mantiene visible como experimental
    CHAMPION_BY_ASSET = {
    ...
    # GUARDA: ningún activo puede quedar sin estrategia visible. Publicar Oro dejó SPX500 en
    # cero y el script terminó en éxito — el dashboard mostraba un activo vacío y nadie se
    # enteraba. Un publisher que puede vaciar un activo debe negarse a terminar bien.
    orphaned = []
    for asset, champ in CHAMPION_BY_ASSET.items():
        live = [s for s in reg["strategies"]
                if s.get("asset_id") == asset and s.get("status") != "archived"]
        if not live:
            orphaned.append(f"{asset} (esperaba {champ})")
    if orphaned:
        print(f"[registry] ERROR: activos SIN estrategia visible: {', '.join(orphaned)}")
        print("[registry] revisa CHAMPION_BY_ASSET y que el bundle de esa campeona exista")
        return 1
    print(f"[registry] OK: los {len(CHAMPION_BY_ASSET)} activos tienen estrategia visible")
    return 0
