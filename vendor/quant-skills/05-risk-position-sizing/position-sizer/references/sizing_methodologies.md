...
- Half Kelly = **18.5%**
- On a $100,000 account that is a risk budget of **$18,500 on a single trade**

**This is exactly where the formula must stop being followed literally.** This
document states elsewhere that risking more than 2% per trade is dangerous and
that 1% is the working rule. 18.5% contradicts that by an order of magnitude.

Both statements are correct, and the tension is the point: Kelly is optimal
*only when W and R are known exactly*. They are estimates from a finite sample.
Kelly is convex in that estimation error, so an optimistic edge estimate
oversizes far more than a pessimistic one undersizes. A 55% win rate measured
over 40 trades has a confidence interval wide enough to contain 45% — and at
W=0.45, R=2.5, true Kelly is *negative*.

So the sizer treats Kelly as an **upper bound to be discounted, never a
target**. `calculate_kelly` reports the raw 18.5% for transparency and caps
what it actually uses at `kelly_cap_pct` (default 2.0%):

```
$100,000 account, W=0.55, R=2.5
  Full Kelly     37.00%
  Half Kelly     18.50%   <- reported, not used
  Applied         2.00%   -> risk budget $2,000
```

Override with `--kelly-cap-pct` only when you have a large, out-of-sample
validated sample — and read `04-backtesting-validation/backtest-expert` on what
"validated" requires before you do.
