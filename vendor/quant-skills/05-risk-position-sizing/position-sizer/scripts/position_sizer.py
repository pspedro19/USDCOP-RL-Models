...
class SizingParameters:
    ...
    # Ceiling on the Kelly-derived risk budget, as a percent of account.
    # Defaults to 2.0 to match this skill's own stated rule that >2% risk per
    # trade is dangerous. Raw Kelly is still reported for transparency; this
    # only bounds what gets USED for sizing. Set to None to disable at your
    # own risk - see calculate_kelly for why that is not recommended.
    kelly_cap_pct: float | None = 2.0
...
def calculate_kelly(params: SizingParameters) -> dict:
    """Kelly Criterion calculation, with the applied budget capped.
    ...
    Why the cap exists. Kelly assumes W and R are KNOWN. They are estimates
    from a finite sample, and Kelly is convex in that error, so overestimating
    edge oversizes far more than underestimating undersizes. With W=0.6 and
    R=2, full Kelly is 37% and half-Kelly is 18.5% of the account risked on a
    single trade - which this skill's own documentation calls dangerous above
    2%. A 60% win rate measured over 40 trades has a confidence interval wide
    enough to contain 45%, where the true Kelly is negative.

    `applied_pct` is what sizing uses and respects `kelly_cap_pct`.
    `half_kelly_pct` is reported unchanged so the raw figure stays visible.
    """
    w = params.win_rate
    ...
    cap = params.kelly_cap_pct
    applied_pct = half_kelly_pct if cap is None else min(half_kelly_pct, cap)
    capped = cap is not None and half_kelly_pct > cap

    return {
        "method": "kelly",
        "kelly_pct": round(kelly_pct, 2),
        "half_kelly_pct": round(half_kelly_pct, 2),
        "applied_pct": round(applied_pct, 2),
        "cap_pct": cap,
        "capped": capped,
        "cap_note": (
            f"Half-Kelly {half_kelly_pct:.2f}% capped to {applied_pct:.2f}%. "
            "Kelly assumes the win rate and payoff ratio are known exactly; "
            "they are estimates, and Kelly oversizes sharply when they are "
            "optimistic. Raise kelly_cap_pct only with a large, "
            "out-of-sample-validated sample."
        )
        if capped
        else None,
    }
...
def calculate_position(params: SizingParameters) -> dict:
    ...
    if is_kelly_mode and not has_entry:
        ...
        budget = params.account_size * kelly["applied_pct"] / 100
        result["recommended_risk_budget"] = round(budget, 2)
        result["recommended_risk_budget_pct"] = kelly["applied_pct"]
    ...
    if is_kelly_mode:
        ...
        budget = (
            to_decimal(params.account_size) * to_decimal(kelly["applied_pct"]) / Decimal(100)
        )
...
def generate_markdown_report(result: dict) -> str:
    ...
    if result["mode"] == "budget":
        ...
        lines.append("- Half Kelly (raw): {}%".format(kelly["half_kelly_pct"]))
        lines.append("- **Applied (capped): {}%**".format(kelly["applied_pct"]))
        if kelly.get("cap_note"):
            lines.append("")
            lines.append("> {}".format(kelly["cap_note"]))
        lines.append(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=2.0,
        help=(
            "Ceiling on the Kelly-derived risk budget, as %%%% of account "
            "(default: 2.0, matching this skill's own >2%%%% danger threshold). "
            "Raw Kelly is still reported. Pass a larger value only with a "
            "large, out-of-sample-validated sample"
        ),
    )
    parser.add_argument(
        "--max-sector-pct",
...
def main() -> None:
    ...
    params = SizingParameters(
        ...
        kelly_cap_pct=args.kelly_cap_pct,
