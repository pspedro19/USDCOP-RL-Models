...
class TestFractionalShares:
    ...
    def test_fractional_kelly_shares_with_stop(self):
        ...
        # Raw Kelly is still reported unchanged, for transparency...
        assert kelly["half_kelly_pct"] == 18.5
        # ...but sizing uses the capped figure. 18.5% of an account on one
        # trade is what this skill's own docs call dangerous above 2%.
        assert kelly["capped"] is True
        assert kelly["applied_pct"] == 2.0
        assert result["final_risk_dollars"] == 2.0
        assert result["final_recommended_shares"] == 0.3076

    def test_kelly_cap_can_be_raised_explicitly(self):
        """The cap is a default, not a prohibition - but it must be deliberate."""
        params = SizingParameters(
            account_size=100,
            entry_price=155.0,
            stop_price=148.50,
        ...
        result = calculate_position(params)
        kelly = result["calculations"]["kelly"]
        assert kelly["capped"] is False
        assert kelly["applied_pct"] == 18.5
        assert result["final_recommended_shares"] == 2.8461
        assert result["final_risk_dollars"] == 18.5

    def test_kelly_below_cap_is_untouched(self):
        """A modest Kelly result passes through unchanged.

        w=0.35, R=2 -> Kelly 2.5%, half-Kelly 1.25%, comfortably under the cap.
        """
        params = SizingParameters(
            account_size=100,
            entry_price=155.0,
        ...
        kelly = calculate_position(params)["calculations"]["kelly"]
        assert kelly["half_kelly_pct"] == 1.25
        assert kelly["capped"] is False
        assert kelly["applied_pct"] == kelly["half_kelly_pct"]
        assert kelly["cap_note"] is None
...
class TestValidation:
    ...
    def test_risk_pct_and_kelly_mutual_exclusive(self):
        ...
        script = str(
            Path(__file__).resolve().parents[1] / "position_sizer.py"
        )
