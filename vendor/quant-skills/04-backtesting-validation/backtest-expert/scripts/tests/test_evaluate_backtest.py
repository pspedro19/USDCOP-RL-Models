...
class TestOverallVerdict:
    def test_deploy_verdict(self, evaluator_module):
        """Score >= 70 AND positive out-of-sample evidence -> Deploy."""
        assert evaluator_module.get_verdict(75, oos_sharpe=1.2) == "Deploy"
        assert evaluator_module.get_verdict(100, oos_sharpe=1.2) == "Deploy"

    def test_high_score_without_oos_cannot_deploy(self, evaluator_module):
        """The core guarantee: an in-sample-only result never reaches Deploy.

        A 94/100 curve fit previously returned Deploy. Every scored dimension
        is in-sample, so without out-of-sample evidence the question of whether
        the edge is real has not been asked.
        """
        assert evaluator_module.get_verdict(94) == "Refine"
        assert evaluator_module.get_verdict(100) == "Refine"

    def test_negative_oos_is_abandoned(self, evaluator_module):
        assert evaluator_module.get_verdict(95, oos_sharpe=-0.3) == "Abandon"
        assert evaluator_module.get_verdict(95, oos_sharpe=0.0) == "Abandon"

    def test_oos_decay_beyond_half_blocks_deploy(self, evaluator_module):
        assert evaluator_module.get_verdict(95, oos_sharpe=0.5, is_sharpe=1.6) == "Refine"
        assert evaluator_module.get_verdict(95, oos_sharpe=1.0, is_sharpe=1.6) == "Deploy"

    def test_multiple_testing_haircut_blocks_deploy(self, evaluator_module):
        """A modest OOS Sharpe does not survive 500 mined variants."""
        assert evaluator_module.get_verdict(95, oos_sharpe=1.4, num_trials=500) == "Refine"
        assert evaluator_module.get_verdict(95, oos_sharpe=2.2, num_trials=500) == "Deploy"
        # Few trials, no meaningful haircut.
        assert evaluator_module.get_verdict(95, oos_sharpe=1.4, num_trials=5) == "Deploy"

    def test_low_score_still_abandoned_regardless_of_oos(self, evaluator_module):
        assert evaluator_module.get_verdict(30, oos_sharpe=3.0) == "Abandon"
...
class TestRedFlagsDetection:
    ...
    def test_clean_backtest_no_flags(self, evaluator_module):
        """Well-constructed backtest WITH out-of-sample evidence has no red flags."""
        result = evaluator_module.evaluate(
            total_trades=200,
            win_rate=60,
            avg_win_pct=2.0,
    ...
    def test_missing_oos_raises_a_high_severity_flag(self, evaluator_module):
        """The same backtest without out-of-sample input is flagged, not cleared."""
        result = evaluator_module.evaluate(
            total_trades=200,
            win_rate=60,
            avg_win_pct=2.0,
        ...
        assert "no_oos_evidence" in ids
        assert result["verdict"] != "Deploy"

    def test_many_trials_flagged(self, evaluator_module):
        result = evaluator_module.evaluate(
            total_trades=200,
            win_rate=60,
            avg_win_pct=2.0,
        ...
        assert "multiple_testing" in [f["id"] for f in result["red_flags"]]
